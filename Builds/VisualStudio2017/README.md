...
We do not recommend Windows for sunnycoind production use at this time. Currently,
...
modification, build sunnycoind under Visual Studio, and run the unit tests you will
...
Any version of Visual Studio 2017 may be used to build sunnycoind. The **Visual
...
Building sunnycoind requires **protoc.exe** version 2. Version 3 is not currently
...
may download a
[protoc.exe](https://sunnycoin.github.io/Downloads/protoc/2.5.1/protoc.exe)
([alternate
link](https://github.com/sunnycoin/Downloads/raw/gh-pages/protoc/2.5.1/protoc.exe))
precompiled Windows executable from the [SUNNYCoin
Organization](https://github.com/sunnycoin).
...
* NOTE: Since sunnycoind links statically to OpenSSL, it does not matter where the
  OpenSSL .DLL files are placed, or what version they are. sunnycoind does not use
...
The sunnycoind application is linked statically to the standard runtimes and
...
process and clone `git@github.com:sunnycoin/sunnycoind.git`. Otherwise follow this
...
above. Navigate to the directory where you want to clone sunnycoind (git bash uses
...
listed on [sunnycoind's GitHub page](https://github.com/sunnycoin/sunnycoind/releases).
...
5. Select either the `sunnycoind.exe` (unity) or `sunnycoind_classic.exe` (non-unity)
   ...
   sunnycoind repo, e.g. `datagen.vcxproj`. Please ignore them.
...
```
.\build\x64-Release\Release\sunnycoind(_classic).exe
```
or
```
.\build\x64-Debug\Debug\sunnycoind(_classic).exe
```
...
execute the following commands within your `sunnycoind` cloned repository:
...
Navigate to the `build\cmake` folder created above and select the `sunnycoind.sln`
file. You can then choose whether to build the `Debug` or `Release` solution
configuration. Within the **Solution Explorer**, selected either the `sunnycoind`
(unity build) project or the `sunnycoind_classic` (non-unity) project, and
right-click to build.

The executable will be in 
```
.\build\cmake\Release\sunnycoind(_classic).exe
```
 or
````
.\build\cmake\Debug\sunnycoind(_classic).exe
````
where these paths are relative to your cloned git repository.

# Unit Test (Recommended)

`sunnycoind` builds a set of unit tests into the server executable. To run these
unit tests after building, pass the `--unittest` option to the compiled
`sunnycoind` executable. The executable will exit with summary info after running
the unit tests.
