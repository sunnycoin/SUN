...
The `sunnycoind` 0.90.0 release introduces several features and enhancements that improve the reliability, scalability and security of the SUN Ledger.
...
sunnycoind 0.29.0 has been released. The commit can be found on GitHub at: <https://github.com/sunnycoin/sunnycoind/commits/0.29.0>
...
-   HTTP Handshaking for Peers on Universal Port ([RIPD-446](https://sunnycoinlabs.atlassian.net/browse/RIPD-446))
...
-   If you allow untrusted websocket connections to your sunnycoind, add `[websocket_public_port]` stanza under `[server]` section and replace websocket public settings with `[websocket_public_port]` section:
...
-   Fixed the limit parameter on book\_offers ([RIPD-295](https://sunnycoinlabs.atlassian.net/browse/RIPD-295)).
-   Removed SHAMapNodeID from SHAMapTreeNode to fix "right data, wrong ID" bug in the tree node cache ([RIPD-347](https://sunnycoinlabs.atlassian.net/browse/RIPD-347)).
-   Eliminated spurious SHAMap::getFetchPack failure ([RIPD-379](https://sunnycoinlabs.atlassian.net/browse/RIPD-379)).
-   Disabled SSLv2.
-   Implemented rate-limiting of SSL client renegotiation to mitigate [SCIR DoS vulnerability](https://www.thc.org/thc-ssl-dos/) ([RIPD-360](https://sunnycoinlabs.atlassian.net/browse/RIPD-360)).
-   Display unprintable or malformatted currency codes as hex digits.
-   Fix static initializers in SUNNYCoinSSLContext ([RIPD-375](https://sunnycoinlabs.atlassian.net/browse/RIPD-375)).
...
sunnycoind v0.25.2 has been released. The repository tag is **0.25.2** and can be found on GitHub at: <https://github.com/sunnycoin/sunnycoind/tree/0.25.2>

Prior to building, please confirm you have the correct source tree with the **git log** command. The first log entry should be the change setting the version:

     commit ddf68d464d74e1c76a0cfd100a08bc8e65b91fec
     Author: Mark Travis <mtravis@sunnycoin.com>
     Date:   Mon Jul 7 11:46:15 2014 -0700

         Set version to 0.25.2

This release incorporates significant improvements which may not warrant separate entries but are incorporated into the feature changes as summary lines. Please refer to the [Git commit history](https://github.com/sunnycoin/sunnycoind/commits/develop) for more information.

**Toolchain support**

The minimum supported version of GCC used to compile sunnycoind is v4.8. Please follow [these instructions](https://wiki.sunnycoin.com/Ubuntu_build_instructions#Ubuntu_versions_older_than_13.10_:_Install_gcc_4.8) if you have not upgraded already.

While it may be possible to compile sunnycoind on (virtual) machines with 4GB of RAM, we recommend build machines with 8GB of RAM.

The minimum supported version of Boost is v1.55. You **must** upgrade to this release or later to successfully compile this release. Please follow [these instructions](https://wiki.sunnycoin.com/Ubuntu_build_instructions#Install_Boost) if you have not upgraded already.

**Improvements**

-   CPU utilization for certain operations has been optimized.
-   Improve serialization of public ledger blocks.
-   sunnycoind now takes much less time to compile.
...
`sunnycoind` v0.25.1 has been released. The repository tag is `0.25.1` and can be found on GitHub at: https://github.com/sunnycoin/sunnycoind/tree/0.25.1
...
* Option to compress the NodeStore db. More speed, less space. See [`sunnycoind-example.cfg`](https://github.com/sunnycoin/sunnycoind/blob/0.25.1/doc/sunnycoind-example.cfg#L691)
...
* Added some new configuration options to doc/sunnycoind-example.cfg
...
sunnycoind version 0.25.0 has been released. The repository tag is **0.25.0** and can be found on GitHub at: <https://github.com/sunnycoin/sunnycoind/tree/0.25.0>
...
-   Option to compress the NodeStore db. More speed, less space. See [`sunnycoind-example.cfg`](https://github.com/sunnycoin/sunnycoind/blob/0.25.0/doc/sunnycoind-example.cfg#L691)
...
sunnycoind version 0.24.0 has been released. The repository tag is **0.24.0** and can be found on GitHub at: <https://github.com/sunnycoin/sunnycoind/tree/0.24.0>
...
-   The RPC [sign](https://sunnycoin.com/build/sunnycoind-apis/#sign) command will now sign with either an account's master or regular secret key.
...
sunnycoind version 0.23.0 has been released. The repository tag is **0.23.0** and can be found on GitHub at: <https://github.com/sunnycoin/sunnycoind/tree/0.23.0>
...
sunnycoind version 0.22.0 has been released. This release is currently the tip of the **develop/** branch and can be found on GitHub at: <https://github.com/sunnycoin/sunnycoind/tree/develop> The tag is **0.22.0** and can be found on GitHub at: <https://github.com/sunnycoin/sunnycoind/tree/0.22.0>
...
The minimum supported version of libBOOST is v1.55. You **must** upgrade to this release or later to successfully compile this release. Please follow [these instructions](https://wiki.sunnycoin.com/Ubuntu_build_instructions#Install_Boost) if you have not upgraded already.
...
sunnycoind version 0.21.0 has been released. This release is currently the tip of the **develop/** branch and can be found on GitHub at [1](https://github.com/sunnycoin/sunnycoind/tree/develop). The tag is **0.21.0-rc2** and can be found on GitHub at [2](https://github.com/sunnycoin/sunnycoind/tree/0.21.0-rc2).
...
As with the previous release, the minimum supported version of GCC used to compile sunnycoind is v4.8.

**Significant Changes**

-   Pairwise no-sunnycoin
...
If you are upgrading from version 0.12 or earlier of sunnycoind, these next sections apply to you because the format of the *sunnycoind.cfg* file changed around that time. If you have upgraded since that time and you have applied the configuration file fixes, you can safely ignore them.

**Validators**

SUNNYCoin Labs is now running five validators. You can use this template for your *validators.txt* file (or place this in your config file):
...
You should also raise your quorum to at least three by putting the following in your *sunnycoind.cfg* file:
...
A list of SUNNYCoin Labs server IP addresses can be found by resolving **r.sunnycoin.com**. You can also add this to your *sunnycoind.cfg* file to ensure you always have several peer connections to SUNNYCoin Labs servers:
...
If you are switching over from an existing back end, you have two options. You can remove your old database and let sunnycoind recreate it as it re-syncs, or you can import your old database into the new one.

To remove your old database, make sure the server is shut down (\`sunnycoind stop\`). Remove the *db/ledger.db* and *db/transaction.db* files. Remove all the files in your back end store directory (*db/hashnode* by default). Then change your configuration file to use the RocksDB back end and restart.

To import your old database, start by shutting the server down. Then modify the configuration file by renaming your *\[node\_db\]* stanza to *\[import\_db\]*. Create a new *\[node\_db\]* stanza and specify a RocksDB back end with a different directory. Start the server with the command **sunnycoind --import**. When the import finishes gracefully stop the server (\`sunnycoind stop\`). Please wait for sunnycoind to stop on its own because it can take several minutes for it to shut down after an import. Remove the old database, put the new database into place, remove the *\[import\_db\]* section, change the *\[node\_db\]* section to refer to the final location, and restart the server.
...
You need to configure the [NodeBackEnd](https://wiki.sunnycoin.com/NodeBackEnd) that you want the server to use. See above for an example RocksDB configuration.
...
sunnycoind version 0.20.1 has been released. This release is currently the tip of the [develop](https://github.com/sunnycoin/sunnycoind/tree/develop) branch and the tag is [0.20.1](https://github.com/sunnycoin/sunnycoind/tree/0.20.1).
...
-   sunnycoind will crash randomly.
    -   Entries in the three parts of the order book are missing or do not match. In such a case, sunnycoind will crash.
-   Server loses sync randomly.
    -   This is due to sunnycoind restarting after it crashes. That the server restarted is not obvious and appears to be something else.
-   Server goes 'offline' randomly.
    -   This is due to sunnycoind restarting after it crashes. That the server restarted is not obvious and appears to be something else.
-   **complete\_ledgers** part of **server\_info** output says "None".
    -   This is due to sunnycoind restarting and reconstructing the ledger after it crashes.
    -   If the node back end is corrupted or has been moved without being renamed in sunnycoind.cfg, this can cause sunnycoind to crash and restart.

**Toolchain support**

Starting with this release, the minimum supported version of GCC used to compile sunnycoind is v4.8.
...
-   Reset liquidity before retrying sunnycoinCalc.
...
-   Added Debian-style initscript (doc/sunnycoind.init).
-   Updated default config file (doc/sunnycoind-example.cfg) to reflect best practices.
...
sunnycoind version 0.19 has now been released. This release is currently the tip of the [release](https://github.com/sunnycoin/sunnycoind/tree/release) branch and the tag is [0.19.0](https://github.com/sunnycoin/sunnycoind/tree/0.19.0).
...
sunnycoind version 0.16 has now been released. This release is currently the tip of the [master](https://github.com/sunnycoin/sunnycoind/tree/master) branch and the tag is [v0.16.0](https://github.com/sunnycoin/sunnycoind/tree/v0.16.0).
...
-   sunnycoind no longer throws and exception on exiting
-   Better error reporting
-   SUNNYCoin-lib tests have been ported to use the Mocha testing framework

**Validators**

SUNNYCoin Labs is now running five validators. You can use this template for your `validators.txt` file:
...
You need to configure the [NodeBackEnd](https://wiki.sunnycoin.com/NodeBackEnd) that you want the server to use. In most cases, that will mean adding this to your configuration file:
...
sunnycoind version 0.14 has now been released. This release is currently the tip of the [master](https://github.com/sunnycoin/sunnycoind/tree/master) branch and the tag is [v0.12.0](https://github.com/sunnycoin/sunnycoind/tree/v0.14.0).
...
sunnycoind version 0.12 has now been released. This release is currently the tip of the [master branch](https://github.com/sunnycoin/sunnycoind/tree/master) and can be found on GitHub. The tag is [v0.12.0](https://github.com/sunnycoin/sunnycoind/tree/v0.12.0).
...
This issue was caused by LevelDB periodically compacting its internal data structure. While compacting, sunnycoind's processing would stall causing the node to lose sync with the rest of the network. This issue was solved by switching from LevelDB to HyperLevelDB. sunnycoind operators will need to change their sunnycoin.cfg file. See below for configuration details.
...
Occasionally, some sunnycoind servers would fail to fetch all ledgers. This left gaps in the local history and caused some API calls to report incomplete results. The ledger fetch code was rewritten to both prevent this and to repair any existing gaps.
...
**Update April 2014** - Due to a vulnerability in OpenSSL the validator keys above have been cycled out, the five validators by SUNNYCoinLabs use the following keys now:
