...
#include <sunnycoin/beast/utility/temp_dir.h>
#include <sunnycoin/resource/ResourceManager.h>
#include <sunnycoin/resource/impl/Entry.h>
#include <sunnycoin/resource/impl/Tuning.h>
#include <sunnycoin/rpc/impl/Tuning.h>

namespace sunnycoin {

class NoSUNNYCoinCheck_test : public beast::unit_test::suite
{
    void
    testBadInput ()
    {
        testcase ("Bad input to nosunnycoin_check");
        ...
        { // missing account field
            auto const result = env.rpc ("json", "nosunnycoin_check", "{}")
                [jss::result];
    ...
    {
        testcase << "Request nosunnycoin_check for " <<
        ...
        {
            env (fset (alice, asfDefaultSUNNYCoin));
            env (trust (alice, gw["USD"](100)));
        }
        else
        {
            env (fclear (alice, asfDefaultSUNNYCoin));
            env (trust (alice, gw["USD"](100), gw, tfSetNoSUNNYCoin));
        }
        env.close ();
        ...
        params[jss::ledger] = "current";
        auto result = env.rpc ("json", "nosunnycoin_check",
            boost::lexical_cast<std::string>(params)) [jss::result];
        ...
        result = env.rpc ("json", "nosunnycoin_check",
            boost::lexical_cast<std::string>(params)) [jss::result];
...
class NoSUNNYCoinCheckLimits_test : public beast::unit_test::suite
{
    void
    testLimits(bool admin)
    {
        ...
        auto checkBalance = [&env]()
        {
            ...
            // cases.
            using namespace sunnycoin::Resource;
        ...
        for (auto i = 0; i < sunnycoin::RPC::Tuning::noSUNNYCoinCheck.rmax + 5; ++i)
        {
            if (! admin)
                checkBalance();
            ...
            env.memoize(gw);
            env (pay (env.master, gw, SUN(1000)),
                seq (autofill),
                fee (txq.getMetrics(*env.current())->expFeeLevel + 1),
                sig (autofill));
            env (fset (gw, asfDefaultSUNNYCoin),
                seq (autofill),
                fee (txq.getMetrics(*env.current())->expFeeLevel + 1),
                sig (autofill));
...
BEAST_DEFINE_TESTSUITE(NoSUNNYCoinCheck, app, sunnycoin);
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(NoSUNNYCoinCheckLimits, app, sunnycoin, 1);
