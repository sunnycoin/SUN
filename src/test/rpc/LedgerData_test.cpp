...
public:
    ...
    void testLedgerType()
    {
        ...
        env(offer(Account{"bob0"}, USD(100), SUN(100)));
...

        {
            ...
            jv[jss::Amount] = SUN(50).value().getJson(0);
...

        {
            ...
            jv[jss::Amount] = SUN(100).value().getJson (0);
        ...
        BEAST_EXPECT( checkArraySize(jrr[jss::state], 1) );
        for (auto const& j : jrr[jss::state])
            BEAST_EXPECT( j["LedgerEntryType"] == "SUNNYCoinState" );
        }
...
BEAST_DEFINE_TESTSUITE_PRIO(LedgerData,app,sunnycoin,1);
