...
public:
    void testErrors()
    {
       ...
       // test errors on marker
        {
            Account const gw{ "G" };
            env.fund(SUN(1000), gw);
            auto const USD = gw["USD"];
            env.trust(USD(1000), bob);
            env(pay(gw, bob, SUN(1)));
            env(offer(bob, SUN(100), bob["USD"](1)), txflags(tfPassive));
    ...
    void testUnsteppedThenStepped()
    {
        ...
        env(offer(bob, SUN(100), bob["USD"](1)),txflags(tfPassive));
        env(offer(bob, SUN(100), USD1(1)), txflags(tfPassive));
    ...
    void testObjectTypes()
    {
        ...
        }
        {
            ...
            jvEscrow[jss::Amount] = SUN(100).value().getJson(0);
        ...
        }
        {
            ...
            jvPayChan[jss::Amount] = SUN (300).value().getJson (0);
...
BEAST_DEFINE_TESTSUITE(AccountObjects,app,sunnycoin);
