...
class OwnerInfo_test : public beast::unit_test::suite
{
    ...
    testBasic ()
    {
        ...
        env(offer(alice, USD(1), SUN(1000)));
        env.close();

        env(pay(gw, alice, USD(50)));
        env(pay(gw, alice, CNY(50)));
        env(offer(alice, CNY(2), SUN(1000)));
        ...
        if (! BEAST_EXPECT (result[jss::accepted].isMember(jss::sunnycoin_lines)))
            return;
        auto lines = result[jss::accepted][jss::sunnycoin_lines];
        ...
        BEAST_EXPECT (
            offers[0u][sfTakerGets.fieldName] == SUN(1000).value().getJson(0));
        BEAST_EXPECT (
            offers[0u][sfTakerPays.fieldName] == USD(1).value().getJson(0));


        // current ledger entry
        if (! BEAST_EXPECT (result[jss::current].isMember(jss::sunnycoin_lines)))
            return;
        lines = result[jss::current][jss::sunnycoin_lines];
        if (! BEAST_EXPECT (lines.isArray() && lines.size() == 2))
            return;
...
BEAST_DEFINE_TESTSUITE(OwnerInfo,app,sunnycoin);
