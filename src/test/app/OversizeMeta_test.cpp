...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(PlumpBook,tx,sunnycoin,5);
...
BEAST_DEFINE_TESTSUITE(ThinBook, tx, sunnycoin);
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(OversizeMeta,tx,sunnycoin,3);
...
public:
    ...
    oversize(std::size_t n)
    {
        ...
        createOffers(env, USD, n);
        env(pay("alice", gw, USD(billion)));
        env(offer("alice", USD(1), SUN(1)), ter(std::ignore));
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(FindOversizeCross,tx,sunnycoin,50);
