...
#include <sunnycoin/app/tx/apply.h>
#include <sunnycoin/basics/Log.h>
#include <sunnycoin/basics/mulDiv.h>
...
class TxQ_test : public beast::unit_test::suite
{
        ...
        std::uint32_t units, std::uint32_t reserve, std::uint32_t increment)
    {
        ...
        checkMetrics(env, 0,
            2 * (sunnycoin::detail::supportedAmendments().size() + 1),
...
public:
    void testQueue()
    {
        ...
        env.fund(SUN(50000), nosunnycoin(alice, bob, charlie, daria));
        ...
        env.fund(SUN(50000), nosunnycoin(elmo, fred, gwen, hank));
    ...
    void testTecResult()
    {
        ...
        // Create accounts
        env.fund(SUN(50000), nosunnycoin(alice, gw));
        ...
        env(offer(alice, SUN(1000), USD(1000)), ter(tecUNFUNDED_OFFER));
        ...
        env(offer(alice, SUN(1000), USD(1000)), ter(terQUEUED));
    ...
    void testLocalTxRetry()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(SUN(50000), nosunnycoin(alice, bob, charlie));
    ...
    void testLastLedgerSeq()
    {
        ...
        env.fund(SUN(1000), nosunnycoin(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(SUN(1000), nosunnycoin(charlie, daria));
        env.close(env.now() + 5s, 10000ms);
        env.fund(SUN(1000), nosunnycoin(edgar, felicia));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testZeroFeeTxn()
    {
        ...
        env.fund(SUN(1000), nosunnycoin(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(SUN(1000), nosunnycoin(carol));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testPreclaimFailures()
    {
        ...
        env.fund(SUN(1000), nosunnycoin(alice));
        ...
        // Fail in preflight
        env(pay(alice, bob, SUN(-1000)),
            ter(temBAD_AMOUNT));

        // Fail in preclaim
        env(noop(alice), fee(SUN(100000)),
            ter(terINSUF_FEE_B));
    }
    ...
    void testMultiTxnPerAccount()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(drops(2000), nosunnycoin(alice));
        env.fund(SUN(500000), nosunnycoin(bob, charlie, daria));
        ...
        aliceFee = env.le(alice)->getFieldAmount(sfBalance).sun().drops()
            - (59);
        env(noop(alice), seq(aliceSeq),
        ...
        // Alice is broke
        env.require(balance(alice, SUN(0)));
    ...
    void testDisabled()
    {
        ...
        env.fund(SUN(50000), nosunnycoin(alice));
    ...
    void testUnexpectedBalanceChange()
    {
        ...
        env.fund(drops(5000), nosunnycoin(alice));
        env.fund(SUN(50000), nosunnycoin(bob));
        ...
        /* At this point, Alice's transaction is indefinitely
            ...
            will get more SUN, and it'll process.
        */
    ...
    void testBlockers()
    {
        ...
        env.fund(SUN(50000), nosunnycoin(alice, bob));
        env.memoize(charlie);
        env.memoize(daria);
    ...
    void testInFlightBalance()
    {
        ...
        env.fund(SUN(50000), nosunnycoin(alice, charlie), gw);
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        //////////////////////////////////////////
        // Offer with high SUN out blocks later txs
        auto aliceSeq = env.seq(alice);
        auto aliceBal = env.balance(alice);

        env.require(balance(alice, SUN(50000)),
            owners(alice, 0));

        // If this offer crosses, all of alice's
        // SUN will be taken (except the reserve).
        env(offer(alice, BUX(5000), SUN(50000)),
            queued);
        ...
        // has plenty of SUN, because the offer didn't
        // cross (of course).
        env.require(balance(alice, aliceBal - drops(20)),
            owners(alice, 1));

        //////////////////////////////////////////
        // Offer with low SUN out allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 8, 5, 4, 256);
        aliceSeq = env.seq(alice);
        ...
        // of alice's SUN will be taken.
        env(offer(alice, BUX(50), SUN(500)),
            queued);
        ...
        //////////////////////////////////////////
        // Large SUN payment blocks later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 10, 6, 5, 256);
        ...
        // (minus the reserve).
        env(pay(alice, charlie, SUN(50000)),
            queued);
        ...
        //////////////////////////////////////////
        // Small SUN payment allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 12, 7, 6, 256);
        ...
        env.require(balance(alice, aliceBal - SUN(500) - drops(20)),
            owners(alice, 2));
        ...
        // alice's SUN balance (other than the fee, of course).
        env(noop(alice), seq(aliceSeq + 1), queued);
        ...
        // SUN balance, but her USD balance went to charlie.
        env.require(balance(alice, aliceBal - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Large SUN to IOU payment blocks later txs.

        env(offer(gw, SUN(500000), USD(50000)));
        // Close so we don't have to deal
        // with tx ordering in consensus.
        env.close();
        ...
        // entire SUN balance to charlie in the
        // form of USD.
        BEAST_EXPECT(SUN(60000) > aliceBal);
        env(pay(alice, charlie, USD(1000)),
            sendmax(SUN(60000)), queued);
        ...
        // to charlie using only a portion of her SUN balance
        env.require(balance(alice, aliceBal - SUN(10000) - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Small SUN to IOU payment allows later txs.
        ...
        // a portion of her SUN balance to charlie
        // in the form of USD.
        BEAST_EXPECT(aliceBal > SUN(6001));
        env(pay(alice, charlie, USD(500)),
            sendmax(SUN(6000)), queued);
        ...
        // So once we close the ledger, alice sent a payment
        // to charlie using only a portion of her SUN balance
        env.require(balance(alice, aliceBal - SUN(5000) - drops(20)),
            balance(alice, USD(0)),
    ...
    void testExpirationReplacement()
    {
        ...
        */
        ...
        env.fund(SUN(500000), nosunnycoin(alice, bob));
        checkMetrics(env, 0, boost::none, 2, 1, 256);
    ...
    void testServerSubscribe()
    {
        ...
        env.fund(SUN(50000), nosunnycoin(a,b,c,d));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        env.fund(SUN(50000), nosunnycoin(e,f,g,h,i));
    ...
    void testClearQueuedAccountTxs()
    {
        ...
        checkMetrics(env, 0, boost::none, 0, 3, 256);
        env.fund(SUN(50000000), alice, bob);
        ...
        testcase("clear queue failure (load)");
        {
            ...
            env(noop(alice), fee(SUN(1)), seq(aliceSeq++), ter(terQUEUED));
...
BEAST_DEFINE_TESTSUITE_PRIO(TxQ,app,sunnycoin,1);
