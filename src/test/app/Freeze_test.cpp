...
#include <sunnycoin/protocol/SField.h>
...
class Freeze_test : public beast::unit_test::suite
{
    ...
    void testSUNNYCoinState(FeatureBitset features)
    {
        testcase("SUNNYCoinState Freeze");
        ...
        env.fund(SUN(1000), G1, alice, bob);
        env.close();
        ...
        env(offer(alice, SUN(500), G1["USD"](100)));
        env.close();
    ...
    testGlobalFreeze(FeatureBitset features)
    {
        ...
        env.fund(SUN(12000), G1);
        env.fund(SUN(1000), A1);
        env.fund(SUN(20000), A2, A3, A4);
        env.close();
        ...
        env(offer(G1, SUN(10000), G1["USD"](100)), txflags(tfPassive));
        env(offer(G1, G1["USD"](100), SUN(10000)), txflags(tfPassive));
        env(offer(A1, SUN(10000), G1["USD"](100)), txflags(tfPassive));
        env(offer(A2, G1["USD"](100), SUN(10000)), txflags(tfPassive));
        env.close();
...

        {
            ...
            env(offer(A3, G1["BTC"](1), SUN(1)));

            //    test: assets can be sold on the market
            env(offer(A4, SUN(1), G1["BTC"](1)));
...

        {
            ...
            env(offer(A3, G1["BTC"](1), SUN(1)), ter(tecFROZEN));

            //    test: assets can't be sold on the market
            env(offer(A4, SUN(1), G1["BTC"](1)), ter(tecFROZEN));
        }
    ...
    testOffersWhenFrozen(FeatureBitset features)
    {
        ...
        env.fund(SUN(1000), G1, A3, A4);
        env.fund(SUN(2000), A2);
        env.close();
        ...
        env(offer(A3, SUN(1000), G1["USD"](1000)), txflags(tfPassive));
        env.close();

        // removal after successful payment
        //    test: make a payment with partially consuming offer
        env(pay(A2, G1, G1["USD"](1)), paths(G1["USD"]), sendmax(SUN(1)));
        env.close();
        ...
        env(offer(A4, SUN(999), G1["USD"](999)));
        env.close();
        ...
        env(offer(A2, G1["USD"](999), SUN(999)));
        affected = env.meta()->getJson(0)[sfAffectedNodes.fieldName];
...
BEAST_DEFINE_TESTSUITE(Freeze, app, sunnycoin);
} // sunnycoin
