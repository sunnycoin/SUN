...
/*
    ...
    Copyright (c) 2017 SUNNYCoin Labs Inc.
...
struct DepositAuth_test : public beast::unit_test::suite
{
    ...
    void testPayIOU()
    {
        // Exercise IOU payments and non-direct SUN payments to an account
        ...
        env (offer (carol, USD(100), SUN(100)));
        env.close();
        ...
        auto failedIouPayments = [this, &env, &alice, &bob, &USD] ()
        {
            ...
            PrettyAmount const bobJrpBalance {env.balance (bob, SUN)};
            PrettyAmount const bobUsdBalance {env.balance (bob, USD)};

            env (pay (alice, bob, USD (50)), ter (tecNO_PERMISSION));
            env.close();

            // Note that even though alice is paying bob in SUN, the payment
            ...
            BEAST_EXPECT (bobJrpBalance == env.balance (bob, SUN));
            BEAST_EXPECT (bobUsdBalance == env.balance (bob, USD));
        };

        //  Test when bob has an SUN balance > base reserve.
        failedIouPayments();

        // Set bob's SUN balance == base reserve.  Also demonstrate that
        // bob can make payments while his lsfDepositAuth flag is set.
        env (pay (bob, alice, USD(25)));
        env.close();

        {
            STAmount const bobPaysSUN {
                env.balance (bob, SUN) - reserve (env, 1)};
            SUNAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
            env (pay (bob, alice, bobPaysSUN), fee (bobPaysFee));
            env.close();
        }

        // Test when bob's SUN balance == base reserve.
        BEAST_EXPECT (env.balance (bob, SUN) == reserve (env, 0));
        BEAST_EXPECT (env.balance (bob, USD) == USD(25));
        failedIouPayments();

        // Test when bob has an SUN balance == 0.
        env (noop (bob), fee (reserve (env, 0)));
        env.close ();

        BEAST_EXPECT (env.balance (bob, SUN) == SUN (0));
        failedIouPayments();

        // Give bob enough SUN for the fee to clear the lsfDepositAuth flag.
    ...
    void testPaySUN()
    {
        // Exercise direct SUN payments to an account that has the
        ...
        env.close();
        BEAST_EXPECT (env.balance (bob, SUN) == SUN (10000) - drops(10));

        // bob has more SUN than the base reserve.  Any SUN payment should fail.
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();
        BEAST_EXPECT (env.balance (bob, SUN) == SUN (10000) - drops(10));

        // Change bob's SUN balance to exactly the base reserve.
        {
            STAmount const bobPaysSUN {
                env.balance (bob, SUN) - reserve (env, 1)};
            SUNAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
        ...
        // bob has exactly the base reserve.  A small enough direct SUN
        ...
        BEAST_EXPECT (env.balance (bob, SUN) == reserve (env, 0) + drops(1));
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();

        // Take bob down to a balance of 0 SUN.
        env (noop (bob), fee (reserve (env, 0) + drops(1)));
        env.close ();
        BEAST_EXPECT (env.balance (bob, SUN) == drops(0));
        ...
        env (pay (alice, bob, reserve (env, 0) + drops(0)));
        env.close();
        BEAST_EXPECT (env.balance (bob, SUN) ==
        ...
        // Take bob back down to a zero SUN balance.
        env (noop (bob), fee (env.balance (bob, SUN)));
        env.close();
        BEAST_EXPECT (env.balance (bob, SUN) == drops(0));
        ...
        env (pay (alice, bob, drops(1)));
        env.close();
        BEAST_EXPECT (env.balance (bob, SUN) == drops(1));
    ...
    void testNoSUNNYCoin()
    {
        // It its current incarnation the DepositAuth flag does not change
        // any behaviors regarding rippling and the NoSUNNYCoin flag.
        // Demonstrate that.
        testcase ("No SUNNYCoin");
        ...
        auto testIssuer = [&] (FeatureBitset const& features,
            bool noSUNNYCoinPrev,
            bool noSUNNYCoinNext,
            bool withDepositAuth)
        {
            ...
            env.fund(SUN(10000), gw1, alice, bob);
            env (trust (gw1, alice["USD"](10), noSUNNYCoinPrev ? tfSetNoSUNNYCoin : 0));
            env (trust (gw1, bob["USD"](10), noSUNNYCoinNext ? tfSetNoSUNNYCoin : 0));
            ...
            TER const result = (noSUNNYCoinNext && noSUNNYCoinPrev)
                ? TER {tecPATH_DRY}
                : TER {tesSUCCESS};
            ...
            bool withDepositAuth)
        {
            ...
            env.fund(SUN(10000), gw1, gw2, alice);
            env (trust (alice, USD1(10), noSUNNYCoinPrev ? tfSetNoSUNNYCoin : 0));
            env (trust (alice, USD2(10), noSUNNYCoinNext ? tfSetNoSUNNYCoin : 0));
        ...
        // Test every combo of noSUNNYCoinPrev, noSUNNYCoinNext, and withDepositAuth
        for (int i = 0; i < 8; ++i)
        {
            auto const noSUNNYCoinPrev = i & 0x1;
            auto const noSUNNYCoinNext = i & 0x2;
...
struct DepositPreauth_test : public beast::unit_test::suite
{
    void testEnable()
    {
        ...
        Account const becky {"becky"};
        {
            ...
            env.fund (SUN (10000), alice, becky);
            env.close();
    ...
    void testInvalid()
    {
        ...
        }
        {
            ...
            Json::Value tx {deposit::auth (alice, becky)};
            tx[sfAuthorize.jsonName] = to_string (sunAccount());
        ...
        // carol gets enough SUN to (barely) meet the reserve.
    ...
    void testPayment (FeatureBitset features)
    {
...

        {
            ...
            Env env (*this, features);
            env.fund (SUN (5000), alice, becky, gw);
            env.close();
            ...
            env (offer (alice, SUN (100), USD (100), tfPassive),
            ...
            env (pay (becky, becky, USD (10)),
                path (~USD), sendmax (SUN (10)), ter (expect));
            env.close();
        }
        ...
        if (supportsPreauth)
        {
            ...
            Env env (*this, features);
            env.fund (SUN (5000), alice, becky, carol, gw);
            env.close();
            ...
            // Make SUN and IOU payments from alice to becky.  Should be fine.
            env (pay (alice, becky, SUN (100)));
            ...
            env (pay (alice, becky, SUN (100)), ter (tecNO_PERMISSION));
            ...
            env (pay (becky, alice, SUN (100)), ter (tecNO_PERMISSION));
...
BEAST_DEFINE_TESTSUITE(DepositAuth,app,sunnycoin);
BEAST_DEFINE_TESTSUITE(DepositPreauth,app,sunnycoin);
