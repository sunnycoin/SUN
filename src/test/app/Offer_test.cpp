...
/*
  ...
  Copyright (c) 2012-2017 SUNNYCoin Labs Inc.
...
{
    SUNAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    static auto
    sunMinusFee (jtx::Env const& env, std::int64_t sunAmount)
        -> jtx::PrettyAmount
    {
        using namespace jtx;
        auto feeDrops = env.current()->fees().base;
        return drops (dropsPerSUN<std::int64_t>::value * sunAmount - feeDrops);
    }
        ...
        jtx::Account const & acct_b, std::string const& currency)
    {
        ...
        jvParams[jss::sunnycoin_state][jss::currency] = currency;
        jvParams[jss::sunnycoin_state][jss::accounts] = Json::arrayValue;
        jvParams[jss::sunnycoin_state][jss::accounts].append(acct_a.human());
        jvParams[jss::sunnycoin_state][jss::accounts].append(acct_b.human());
...
public:
    void testRmFundedOffer (FeatureBitset features)
    {
        ...
        // be "taker gets" SUN. Old, broken would remove the first
        // "taker gets" sun offer, even though the offer is still funded and
        ...
        // "taker gets" must be SUN
        // (Different amounts so I can distinguish the offers)
        env (offer (carol, BTC (49), SUN (49)));
        env (offer (carol, BTC (51), SUN (51)));

        // Offers for the poor quality path
        // Must be two offers at the same quality
        env (offer (carol, SUN (50), USD (50)));
        env (offer (carol, SUN (50), USD (50)));

        // Offers for the good quality path
        env (offer (carol, BTC (1), USD (100)));

        PathSet paths (Path (SUN, USD), Path (USD));
        ...
        BEAST_EXPECT(!isOffer (env, carol, BTC (1), USD (100)) &&
            isOffer (env, carol, BTC (49), SUN (49)));
    }
    ...
    void testCanceledOffer (FeatureBitset features)
    {
        ...
        env (offer (alice, SUN (500), USD (100)),
            require (offers (alice, 1)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, SUN (500), USD (100)));

        // cancel the offer above and replace it with a new offer
        auto const offer2Seq = env.seq (alice);

        env (offer (alice, SUN (300), USD (100)),
             json (jss::OfferSequence, offer1Seq),
             require (offers (alice, 1)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, SUN (300), USD (100)) &&
            !isOffer (env, alice, SUN (500), USD (100)));

        // Test canceling non-existent offer.
//      auto const offer3Seq = env.seq (alice);

        env (offer (alice, SUN (400), USD (200)),
             json (jss::OfferSequence, offer1Seq),
             require (offers (alice, 2)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, SUN (300), USD (100)) &&
            isOffer (env, alice, SUN (400), USD (200)));

        // Test cancellation now with OfferCancel tx
        auto const offer4Seq = env.seq (alice);
        env (offer (alice, SUN (222), USD (111)),
            require (offers (alice, 3)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, SUN (222), USD (111)));
        {
        ...
        env.require (offers (alice, 2));
        BEAST_EXPECT( isOffer (env, alice, SUN (300), USD (100))); // offer2
        BEAST_EXPECT(!isOffer (env, alice, SUN   (5), USD   (2))); // expired
    }
    ...
    void testSUNTinyPayment (FeatureBitset features)
    {
        testcase ("SUN Tiny payments");

        // Regression test for tiny sun payments
        // In some cases, when the payment code calculates
        // the amount of sun needed as input to an sun->iou offer
        ...
        // 1) When we calculate the amount of input sun needed for an offer
        //    from sun->iou, the amount is less than 1 drop (after rounding
        ...
        for (auto withFix : {false, true})
        {
            ...
            env.fund (SUN (10000), alice, bob, carol, dan, erin, gw);
    ...
    void testEnforceNoSUNNYCoin (FeatureBitset features)
    {
        testcase ("Enforce No SUNNYCoin");
        ...
        {
            // No sunnycoin with an implied account step after an offer
            Env env {*this, features};
            auto const closeTime =
                fix1449Time () +
                    100 * env.closed ()->info ().closeTimeResolution;
            ...
            env.fund (SUN (10000), alice, nosunnycoin (bob), carol, dan, gw1, gw2);
            env.trust (USD1 (1000), alice, carol, dan);
            env(trust (bob, USD1 (1000), tfSetNoSUNNYCoin));
            env.trust (USD2 (1000), alice, carol, dan);
            env(trust (bob, USD2 (1000), tfSetNoSUNNYCoin));

            env (pay (gw1, dan, USD1 (50)));
            env (pay (gw1, bob, USD1 (50)));
            env (pay (gw2, bob, USD2 (50)));

            env (offer (dan, SUN (50), USD1 (50)));

            env (pay (alice, carol, USD2 (50)), path (~USD1, bob),
                sendmax (SUN (50)), txflags (tfNoSUNNYCoinDirect),
                ter(tecPATH_DRY));
        }
        {
            ...
            env.fund (SUN (10000), alice, bob, carol, dan, gw1, gw2);
            ...
            env.require (balance (alice, sunMinusFee (env, 10000 - 50)));
    ...
    testInsufficientReserve (FeatureBitset features)
    {
        ...
        // No crossing:
        {
            ...
            env (offer (alice, sunOffer, usdOffer),
        ...
        // Partial cross:
        {
            ...
            auto const sunOffer2 = SUN (500);

            env.fund (r + f + sunOffer, bob);
            env (offer (bob, usdOffer2, sunOffer2),   ter(tesSUCCESS));
            env.fund (r + f, alice);
            env (trust (alice, usdOffer),             ter(tesSUCCESS));
            env (pay (gw, alice, usdOffer),           ter(tesSUCCESS));
            env (offer (alice, sunOffer, usdOffer),   ter(tesSUCCESS));

            env.require (
                balance (alice, r - f + sunOffer2),
                balance (alice, usdOffer2),
                owners (alice, 1),
                balance (bob, r + sunOffer2),
        ...
        // buy SUN. If it fully crosses, we succeed.
        {
            Env env {*this, features};
            auto const closeTime =
                fix1449Time () +
            ...
            env.fund (r + f + sunOffer, bob, carol);
            env (offer (bob, usdOffer2, sunOffer2),    ter(tesSUCCESS));
            env (offer (carol, usdOffer, sunOffer),    ter(tesSUCCESS));
            ...
            env.require (
                balance (alice, r - f + sunOffer),
                balance (alice, USD(0)),
                owners (alice, 1),
                balance (bob, r + sunOffer2),
                balance (bob, usdOffer2),
                owners (bob, 1),
                balance (carol, r + sunOffer2),
    ...
    testFillModes (FeatureBitset features)
    {
        ...
        auto const startBalance = SUN (1000000);
        auto const gw = Account {"gateway"};
        auto const alice = Account {"alice"};
        auto const bob = Account {"bob"};
        auto const USD = gw["USD"];
        ...
        // a fee and not place the offer on the books:
        {
            ...
            env (offer (alice, SUN (500), USD (500)),
                txflags (tfFillOrKill),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + SUN (500)),
                balance (alice, USD (500)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - SUN (500)),
                balance (bob, USD (500)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // and add nothing on the books:
        {
            ...
            // Partially cross:
            env (offer (bob, USD (50), SUN (50)),            ter(tesSUCCESS));
            env (offer (alice, SUN (1000), USD (1000)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + SUN (50)),
                balance (alice, USD (950)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - SUN (50)),
                balance (bob, USD (50)),
                owners (bob, 1),
                offers (bob, 0));
            ...
            // Fully cross:
            env (offer (bob, USD (50), SUN (50)),            ter(tesSUCCESS));
            env (offer (alice, SUN (50), USD (50)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f - f + SUN (100)),
                balance (alice, USD (900)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - f - SUN (100)),
                balance (bob, USD (100)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // tfPassive -- place the offer without crossing it.
        {
            ...
            env (offer (alice, USD(1000), SUN(2000)));
            env.close();
            ...
            for (auto offerPtr : aliceOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerGets] == SUN (2000));
            ...
            env (offer (bob, SUN(2000), USD(1000), tfPassive));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == SUN (2000));
            }

            // It should be possible for gw to cross both of those offers.
            env (offer (gw, SUN(2000), USD(1000)));
            env.close();
            env.require (offers (alice, 0));
            env.require (offers (gw, 0));
            env.require (offers (bob, 1));

            env (offer (gw, USD(1000), SUN(2000)));
            env.close();
        ...
        // tfPassive -- cross only offers of better quality.
        {
            ...
            env (pay (gw, "bob", USD(1000)));
            env (offer ("alice", USD(500), SUN(1001)));
            env.close();

            env (offer ("alice", USD(500), SUN(1000)));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == SUN (999));
            }
        }
    }
    ...
    testMalformed(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (1000), SUN (1000)),
        ...
        {
            // Alice tries an SUN to SUN order:
            env (offer (alice, SUN (1000), SUN (1000)),   ter(temBAD_OFFER));
            env.require (
                owners (alice, 0),
                offers (alice, 0));
        ...
        {
            env (offer (alice, -USD (1000), SUN (1000)),  ter(temBAD_OFFER));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        ...
        // Use SUN as a currency code
        {
            auto const BAD = IOU(gw, badCurrency());

            env (offer (alice, SUN (1000), BAD (1000)),   ter(temBAD_CURRENCY));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        }
    ...
    testExpiration(FeatureBitset features)
    {
        ...
        env (offer (bob, usdOffer, sunOffer),     ter(tesSUCCESS));
        env.require (
            balance (alice, startBalance - f - f - f),
            balance (alice, usdOffer),
            offers (alice, 0),
    ...
    testUnfundedCross(FeatureBitset features)
    {
        ...
        env (offer ("alice", usdOffer, sunOffer),     ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env (offer ("bob", usdOffer, sunOffer),       ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env.fund (reserve (env, 0) + f + SUN(1), "carol");
        env (offer ("carol", usdOffer, sunOffer), ter(tecINSUF_RESERVE_OFFER));
        env.require (
            balance ("carol", reserve (env, 0) + SUN(1)),
            owners ("carol", 0));

        // Account has enough for the reserve plus one
        // offer, and the fee.
        env.fund (reserve (env, 1) + f, "dan");
        env (offer ("dan", usdOffer, sunOffer),       ter(tesSUCCESS));
        env.require (
        ...
        env.fund (reserve (env, 1) + f + sunOffer, "eve");
        env (offer ("eve", usdOffer, sunOffer),       ter(tesSUCCESS));
        env.require (
            balance ("eve", reserve (env, 1) + sunOffer),
            owners ("eve", 1));
    }
    ...
    testSelfCross(bool use_partner, FeatureBitset features)
    {
        ...
        env.fund (SUN (10000), gw);
        if (use_partner)
        {
            env.fund (SUN (10000), partner);
        ...
        // through SUN
        env (offer (account_to_test, BTC (250), SUN (1000)),
                 offers (account_to_test, 1));

        // validate that the book now shows a BTC for SUN offer
        BEAST_EXPECT(isOffer(env, account_to_test, BTC(250), SUN(1000)));

        auto const secondLegSeq = env.seq(account_to_test);
        env (offer (account_to_test, SUN(1000), USD (50)),
                 offers (account_to_test, 2));

        // validate that the book also shows a SUN for USD offer
        BEAST_EXPECT(isOffer(env, account_to_test, SUN(1000), USD(50)));
        ...
        // behavior.
        {
            ...
            for (auto const& offerPtr : acctOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == SUN (0));
            }
        }
    ...
    testNegativeBalance(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (50), SUN (150000)));
        ...
        // small_amount and transfer no SUN.  The new offer crossing transfers
        ...
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "50");
        BEAST_EXPECT(env.balance (alice, sunIssue()) ==
        ...
        BEAST_EXPECT(env.balance (bob, sunIssue()) ==
            bob_initial_balance -
    ...
    void
    testOfferCrossWithSUN(bool reverse_order, FeatureBitset features)
    {
        testcase (std::string("Offer Crossing with SUN, ") +
        ...
        env.fund (SUN (10000), gw, alice, bob);
        ...
        if (reverse_order)
            env (offer (bob, USD (1), SUN (4000)));

        env (offer (alice, SUN (150000), USD (50)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                SUN (10000).value ().mantissa () -
                SUN (reverse_order ? 4000 : 3000).value ().mantissa () -
    ...
    testOfferCrossWithLimitOverride(FeatureBitset features)
    {
        ...
        env.fund (SUN (100000), gw, alice, bob);
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                SUN (100000).value ().mantissa () -
                SUN (3000).value ().mantissa () -
    ...
    testOfferAcceptThenCancel(FeatureBitset features)
    {
        ...
        env (offer (env.master, SUN (500), USD (100)));
        env.close ();
    ...
    testCurrencyConversionEntire(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == SUN (500).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == USD (100).value ().getJson (0));

        env (pay (alice, alice, SUN (500)), sendmax (USD (100)));
    ...
    testCurrencyConversionInParts(FeatureBitset features)
    {
        ...
        env (pay (alice, alice, SUN (200)), sendmax (USD (100)));

        // The previous payment reduced the remaining offer amount by 200 SUN
        auto jro = ledgerEntryOffer (env, bob, bobOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == SUN (300).value ().getText ());
        BEAST_EXPECT(
        ...
        // alice now has 200 more SUN from the payment
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                SUN (10000).value ().mantissa () +
                SUN (200).value ().mantissa () -
                env.current ()->fees ().base * 2)
        );
        ...
        // Alice converts USD to SUN which should fail
        // due to PartialPayment.
        env (pay (alice, alice, SUN (600)), sendmax (USD (100)),
            ter(tecPATH_PARTIAL));

        // Alice converts USD to SUN, should succeed because
        ...
        // only 300 SUN should be have been payed since that's all
    ...
    void
    testCrossCurrencyStartSUN(FeatureBitset features)
    {
        testcase ("Cross Currency Payment: Start with SUN");
        ...
        env (offer (carol, SUN (500), USD (50)));

        env (pay (alice, bob, USD (25)), sendmax (SUN (333)));
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == SUN (250).value ().getText ());
    }

    void
    testCrossCurrencyEndSUN(FeatureBitset features)
    {
        testcase ("Cross Currency Payment: End with SUN");
        ...
        auto const carolOfferSeq = env.seq (carol);
        env (offer (carol, USD (50), SUN (500)));

        env (pay (alice, bob, SUN (250)), sendmax (USD (333)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                SUN (10000).value ().mantissa () +
                SUN (250).value ().mantissa ())
        );

        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == SUN (250).value ().getText ());
        BEAST_EXPECT(
    ...
    testCrossCurrencyBridged(FeatureBitset features)
    {
        ...
        env.fund (SUN (10000), gw1, gw2, alice, bob, carol, dan);
        ...
        env (offer (dan, SUN (500), EUR (50)));

        Json::Value jtp {Json::arrayValue};
        jtp[0u][0u][jss::currency] = "SUN";
        ...
        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == SUN (200).value ().getText ());
        BEAST_EXPECT(
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == SUN (200).value ().getText ());
    }
    ...
    testOfferFeesConsumeFunds(FeatureBitset features)
    {
        ...
        //  1 for payment          == 4
        auto const starting_sun = SUN (100) +
            env.current ()->fees ().accountReserve (3) +
            env.current ()->fees ().base * 4;

        env.fund (starting_sun, gw1, gw2, gw3, alice, bob);
        ...
        env (offer (bob, SUN (200), USD1 (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD1 (200), SUN (200)));

        auto jrr = ledgerEntryState (env, alice, gw1, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == SUN (350).value ().getText ()
        );
    ...
    testSellFlagBasic(FeatureBitset features)
    {
        ...
        env.fund (starting_sun, gw, alice, bob);
        ...
        env (offer (bob, SUN (200), USD (200)), json(jss::Flags, tfSell));
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD (200), SUN (200)), json(jss::Flags, tfSell));

        auto jrr = ledgerEntryState (env, alice, gw, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "-100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == SUN (250).value ().getText ()
        );
    ...
    testSellFlagExceedLimit(FeatureBitset features)
    {
        ...
        env (offer (bob, SUN (100), USD (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        // Taker pays 100 USD for 100 SUN.
        // Selling SUN.
        // Will sell all 100 SUN and get more USD than asked for.
        env (offer (alice, USD (100), SUN (100)), json(jss::Flags, tfSell));
    ...
    testGatewayCrossCurrency(FeatureBitset features)
    {
        ...
        auto const starting_sun = SUN (100.1) +
            env.current ()->fees ().accountReserve (1) +
            env.current ()->fees ().base * 2;
        ...
        if (wsc->version() == 2)
        {
            ...
            BEAST_EXPECT(jrr.isMember(jss::sunnycoinrpc) && jrr[jss::sunnycoinrpc] == "2.0");
    ...
    void testPartialCross (FeatureBitset features)
    {
        ...
        env.fund (SUN(10000000), gw);

        // The fee that's charged for transactions
        auto const f = env.current ()->fees ().base;

        // To keep things simple all offers are 1 : 1 for SUN : USD.
        ...
        struct TestData
        {
            ...
            int bookAmount;            // USD -> SUN offer on the books
            preTrustType preTrust;     // If true, pre-establish trust line
            int offerAmount;           // Account offers this much SUN -> USD
...
{"flo",             reserve (env, 1) + 0*f,       1,   noPreTrust,     1000,             tesSUCCESS, SUN(   1) + f, USD(      1),      0,     1},
{"gay",             reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, SUN(  50) + f, USD(     50),      0,     1},
{"hye", SUN(1000)                    + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, SUN( 800) + f, USD(    800),      0,     1},
{"ivy", SUN(   1) + reserve (env, 1) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, SUN(   1) + f, USD(      1),      0,     1},
{"joy", SUN(   1) + reserve (env, 2) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, SUN(   1) + f, USD(      1),      1,     2},
{"kim", SUN( 900) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, SUN( 999) + f, USD(    999),      0,     1},
{"liz", SUN( 998) + reserve (env, 0) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, SUN( 998) + f, USD(    998),      0,     1},
{"meg", SUN( 998) + reserve (env, 1) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, SUN( 999) + f, USD(    999),      0,     1},
{"nia", SUN( 998) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, SUN( 999) + f, USD(    999),      1,     2},
{"ova", SUN( 999) + reserve (env, 0) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, SUN( 999) + f, USD(    999),      0,     1},
{"pam", SUN( 999) + reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, SUN(1000) + f, USD(   1000),      0,     1},
{"rae", SUN( 999) + reserve (env, 2) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, SUN(1000) + f, USD(   1000),      0,     1},
{"sue", SUN(1000) + reserve (env, 2) + 1*f,       0,   noPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
...
{"eli", SUN(  20) + reserve (env, 0) + 1*f,    1000,   gwPreTrust,     1000,             tesSUCCESS, SUN(20) + 1*f, USD(     20),      0,     0},
{"fyn",             reserve (env, 1) + 0*f,       0,   gwPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
{"gar",             reserve (env, 1) + 0*f,       1,   gwPreTrust,     1000,             tesSUCCESS, SUN(   1) + f, USD(      1),      1,     1},
{"hal",             reserve (env, 1) + 1*f,       1,   gwPreTrust,     1000,             tesSUCCESS, SUN(   1) + f, USD(      1),      1,     1},
...
{"ted", SUN(  20) + reserve (env, 1) + 2*f,    1000, acctPreTrust,     1000,             tesSUCCESS, SUN(20) + 2*f, USD(     20),      0,     1},
{"uli",             reserve (env, 2) + 0*f,       0, acctPreTrust,     1000, tecINSUF_RESERVE_OFFER,           2*f, USD(      0),      0,     1},
{"vic",             reserve (env, 2) + 0*f,       1, acctPreTrust,     1000,             tesSUCCESS, SUN( 1) + 2*f, USD(      1),      0,     1},
{"wes",             reserve (env, 2) + 1*f,       0, acctPreTrust,     1000,             tesSUCCESS,           2*f, USD(      0),      1,     2},
{"xan",             reserve (env, 2) + 1*f,       1, acctPreTrust,     1000,             tesSUCCESS, SUN( 1) + 2*f, USD(      1),      1,     2},
};
        ...
        for (auto const& t : tests)
        {
            ...
            if (book)
                env (offer (gw, SUN (book), USD (book)));
            env.close();
            ...
            // to be enough SUN reserve for acct to create the trust line.
            ...
            env (offer (acct, USD (acctOffer), SUN (acctOffer)), ter (t.tec));
            env.close();
            std::uint32_t const acctOfferSeq = env.seq (acct) - 1;

            BEAST_EXPECT (env.balance (acct, USD.issue()) == t.balanceUsd);
            BEAST_EXPECT (
                env.balance (acct, sunIssue()) == t.fundJrp - t.spentJrp);
            ...
            if (acctOffers.size() && t.offers)
            {
                ...
                BEAST_EXPECT (acctOffer[sfTakerGets] == SUN (leftover));
    ...
    void
    testSUNDirectCross (FeatureBitset features)
    {
        testcase ("SUN Direct Crossing");
        ...
        // The scenario:
        //   o alice has USD but wants SUN.
        //   o bob has SUN but wants USD.
        auto const alicesSUN = env.balance (alice);
        auto const bobsSUN = env.balance (bob);

        env (offer (alice, sunOffer, usdOffer));
        env.close();
        env (offer (bob, usdOffer, sunOffer));

        env.close();
        env.require (
            balance (alice, USD(0)),
            balance (bob, usdOffer),
            balance (alice, alicesSUN + sunOffer - fee),
            balance (bob,   bobsSUN   - sunOffer - fee),
            offers (alice, 0),
            offers (bob, 0));
        ...
        env (offer (alice, USD(999), SUN(999)));
        env (offer (bob, sunOffer, usdOffer));
        ...
        verifyDefaultTrustline (env, bob, USD(1));
        {
            ...
            BEAST_EXPECT (bobsOffer[sfTakerPays] == SUN (1));
        }
   }
    ...
    testBridgedCross (FeatureBitset features)
    {
        ...
        env.fund (SUN(1000000), gw, alice, bob, carol);
        env.close();
        ...
        env (offer (alice, SUN(1000), usdOffer));
        env (offer (bob, eurOffer, SUN(1000)));
        ...
        env.require (
            ...
            balance (bob, bobJrpBalance - SUN(400)),
            offers (carol, 0));
        ...
        verifyDefaultTrustline (env, carol, USD(400));
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == SUN (600));
        }
        {
            auto const bobsOffers = offersOnAccount (env, bob);
            BEAST_EXPECT (bobsOffers.size() == 1);
            ...
            BEAST_EXPECT (bobsOffer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (bobsOffer[sfTakerGets] == SUN (600));
        ...
        env.require (
            ...
            balance (bob, bobJrpBalance - SUN(1000)),
            offers (bob, 0),
            offers (carol, 0));
        ...
        if (alicesOffers.size() != 0)
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == SUN (0));
        }
    }
    ...
    testSellOffer (FeatureBitset features)
    {
        ...
        {
            std::string account;       // Account operated on
            STAmount fundJrp;          // SUN acct funded with
            ...
            TestData (
                ...
                STAmount const& fundJrp_,       // SUN acct funded with
...
{"ann", SUN(10) + reserve (env, 0) + 1*f, USD( 0), SUN(10), USD( 5),  USD(10),  SUN(10), tecINSUF_RESERVE_OFFER, SUN( 0) + (1*f),  USD( 0),      0,     0},
{"bev", SUN(10) + reserve (env, 1) + 1*f, USD( 0), SUN(10), USD( 5),  USD(10),  SUN(10),             tesSUCCESS, SUN( 0) + (1*f),  USD( 0),      1,     1,     SUN(10),  USD(10)},
{"cam", SUN(10) + reserve (env, 0) + 1*f, USD( 0), SUN(10), USD(10),  USD(10),  SUN(10),             tesSUCCESS, SUN(10) + (1*f),  USD(10),      0,     1},
{"deb", SUN(10) + reserve (env, 0) + 1*f, USD( 0), SUN(10), USD(20),  USD(10),  SUN(10),             tesSUCCESS, SUN(10) + (1*f),  USD(20),      0,     1},
{"eve", SUN(10) + reserve (env, 0) + 1*f, USD( 0), SUN(10), USD(20),  USD( 5),  SUN( 5),             tesSUCCESS, SUN( 5) + (1*f),  USD(10),      0,     1},
{"flo", SUN(10) + reserve (env, 0) + 1*f, USD( 0), SUN(10), USD(20),  USD(20),  SUN(20),             tesSUCCESS, SUN(10) + (1*f),  USD(20),      0,     1},
{"gay", SUN(20) + reserve (env, 1) + 1*f, USD( 0), SUN(10), USD(20),  USD(20),  SUN(20),             tesSUCCESS, SUN(10) + (1*f),  USD(20),      0,     1},
{"hye", SUN(20) + reserve (env, 2) + 1*f, USD( 0), SUN(10), USD(20),  USD(20),  SUN(20),             tesSUCCESS, SUN(10) + (1*f),  USD(20),      1,     2,     SUN(10),  USD(10)},
// acct pays USD
{"meg",           reserve (env, 1) + 2*f, USD(10), USD(10), SUN( 5),  SUN(10),  USD(10), tecINSUF_RESERVE_OFFER, SUN(  0) + (2*f),  USD(10),      0,     1},
{"nia",           reserve (env, 2) + 2*f, USD(10), USD(10), SUN( 5),  SUN(10),  USD(10),             tesSUCCESS, SUN(  0) + (2*f),  USD(10),      1,     2,     USD(10),  SUN(10)},
{"ova",           reserve (env, 1) + 2*f, USD(10), USD(10), SUN(10),  SUN(10),  USD(10),             tesSUCCESS, SUN(-10) + (2*f),  USD( 0),      0,     1},
{"pam",           reserve (env, 1) + 2*f, USD(10), USD(10), SUN(20),  SUN(10),  USD(10),             tesSUCCESS, SUN(-20) + (2*f),  USD( 0),      0,     1},
{"qui",           reserve (env, 1) + 2*f, USD(10), USD(20), SUN(40),  SUN(10),  USD(10),             tesSUCCESS, SUN(-20) + (2*f),  USD( 0),      0,     1},
{"rae",           reserve (env, 2) + 2*f, USD(10), USD( 5), SUN( 5),  SUN(10),  USD(10),             tesSUCCESS, SUN( -5) + (2*f),  USD( 5),      1,     2,     USD( 5),  SUN( 5)},
{"sue",           reserve (env, 2) + 2*f, USD(10), USD( 5), SUN(10),  SUN(10),  USD(10),             tesSUCCESS, SUN(-10) + (2*f),  USD( 5),      1,     2,     USD( 5),  SUN( 5)},
};
    ...
    testSellWithFillOrKill (FeatureBitset features)
    {
        ...
        env.fund (SUN(10000000), gw, alice, bob);

        // bob offers SUN for USD.
        env (trust(bob, USD(200)));
        env.close();
        env (pay(gw, bob, USD(100)));
        env.close();
        env (offer(bob, SUN(2000), USD(20)));
        env.close();
        {
            ...
            env (offer(alice, USD(21), SUN(2100), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(alice, USD(20), SUN(2000), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(bob, SUN(2000), USD(20)));
            env.close();
            env (offer(alice, USD(10), SUN(1500), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            // We're using bob's left-over offer for SUN(500), USD(5)
            env (offer(alice, USD(1), SUN(501), tfSell | tfFillOrKill));
            env.close();
            env.require (balance (alice, USD(35)));
            env.require (offers (alice, 0));
            env.require (balance (bob, USD(65)));
        }
        {
            ...
            // We're using bob's left-over offer for SUN(500), USD(5)
            env (offer(alice, USD(1), SUN(500), tfSell | tfFillOrKill));
            env.close();
    ...
    testTransferRateOffer (FeatureBitset features)
    {
        ...
        env(rate(gw1, 1.25));
        {
            ...
            auto const bob = Account("bob");
            env.fund (SUN(100) + reserve(env, 2) + (fee*2), ann, bob);
            env.close();
            ...
            // bob offers to sell USD(100) for SUN.  alice takes bob's offer.
            ...
            env (offer (bob, SUN(1), USD(100)));
            env.close();

            env (offer (ann, USD(100), SUN(1)));
            env.close();

            env.require (balance (ann, USD(100)));
            env.require (balance (ann, SUN( 99) + reserve(env, 2)));
            env.require (offers (ann, 0));

            env.require (balance (bob, USD(  0)));
            env.require (balance (bob, SUN(101) + reserve(env, 2)));
            env.require (offers (bob, 0));
        }
        {
            // Reverse the order, so the offer in the books is to sell SUN
            // in return for USD.  Gateway rate should still apply identically.
            auto const che = Account("che");
            auto const deb = Account("deb");
            env.fund (SUN(100) + reserve(env, 2) + (fee*2), che, deb);
            env.close();
            ...
            env (offer (che, USD(100), SUN(1)));
            env.close();

            env (offer (deb, SUN(1), USD(100)));
            env.close();

            env.require (balance (che, USD(100)));
            env.require (balance (che, SUN( 99) + reserve(env, 2)));
            env.require (offers (che, 0));

            env.require (balance (deb, USD(  0)));
            env.require (balance (deb, SUN(101) + reserve(env, 2)));
        ...
        }
        {
            ...
            env.fund (SUN(20000) + fee*2, eve, fyn);
            env.close();
            ...
            env (offer (eve, USD(10), SUN(4000)));
            env.close();
            std::uint32_t const eveOfferSeq = env.seq (eve) - 1;

            env (offer (fyn, SUN(2000), USD(5)));
            env.close();

            env.require (balance (eve, USD(105)));
            env.require (balance (eve, SUN(18000)));
            ...
            if (evesOffers.size() != 0)
            {
                ...
                BEAST_EXPECT (evesOffer[sfTakerGets] == SUN (2000));
            ...
            env.require (balance (fyn, SUN(22000)));
        ...
        env.fund (SUN(100000), gw2);
        env.close();

        env(rate(gw2, 1.5));
        {
            // Remove SUN from the equation.  Give the two currencies two
        ...
        }
        {
            ...
            env.fund (SUN(2) + reserve(env, 3) + (fee*3), ova, pat, qae);
            env.close();

            //   o ova has USD but wants XPR.
            //   o pat has SUN but wants EUR.
            ...
            env (offer (ova, SUN(2), USD(100)));
            env (offer (pat, EUR(100), SUN(2)));
            env.close();
            ...
            env.require (balance (ova, SUN(4) + reserve(env, 3)));
    ...
    testSelfCrossOffer1 (FeatureBitset features)
    {
        ...
        for (auto const& offerPtr : gwOffers)
        {
            auto const& offer = *offerPtr;
            BEAST_EXPECT (offer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (offer[sfTakerGets] == SUN (600));
        ...
        for (auto const& offerPtr : gwOffers)
        {
            ...
            BEAST_EXPECT (offer[sfTakerPays] == SUN (1000));
        }
    }
    ...
    testSelfCrossOffer2 (FeatureBitset features)
    {
        ...
        struct TestData
        {
            ...
            STAmount fundSUN;          // SUN acct funded with
            STAmount fundUSD;          // USD acct funded with
        ...
        TestData const tests[]
        {
// acct               fundSUN    fundUSD    fundEUR           firstOfferTec          secondOfferTec
...
{"flo", reserve(env, 3) +   0, USD(1000), EUR(1000), tecINSUF_RESERVE_OFFER, tecINSUF_RESERVE_OFFER},
    ...
    testSelfIssueOffer (FeatureBitset features)
    {
        ...
        env.fund(SUN(50000) + f, alice, bob);
        env.close();

        env(offer(alice, USD(5000), SUN(50000)));
        env.close();

        // This offer should take alice's offer up to Alice's reserve.
        env(offer(bob, SUN(50000), USD(5000)));
        env.close();

        // alice's offer should have been removed, since she's down to her
        // SUN reserve.
        env.require (balance (alice, SUN(250)));
        ...
        for (auto const& offerPtr : bobOffers)
        {
            ...
            BEAST_EXPECT(offer[sfTakerPays] == SUN (250));
        }
    }
    ...
    void testSelfCrossLowQualityOffer (FeatureBitset features)
    {
        ...
        // for more SUN than the offering account held.  This unit test
    ...
    void testSelfPayXferFeeOffer (FeatureBitset features)
    {
        ...
        //
        //  2. alice makes an offer to buy SUN and sell USD.
        //  3. bob makes an offer to buy BTC and sell SUN.
        //
        ...
        //  o bob' offer which takes alice's BTC and sells SUN to...
        //  o alice's first offer which takes bob's SUN and sells USD to...
        ...
        auto const BTC = gw["BTC"];
        auto const USD = gw["USD"];
        auto const startJrpBalance = SUN (4000000);
        ...
        struct Actor
        {
            ...
            PrettyAmount sun;  // final expected after crossing
...
// { 0, 0, 0, BTC( 5), { {"flo", 0, drops(3999999999950), BTC( 5.0), USD(2000)}                                                      } }  // no xfer fee
        ...
        for (auto const& t : tests)
        {
            ...
            {
                env.fund (SUN (4000000), actor.acct);
                env.close();
            ...
            env (offer (leg0, BTC(10),     SUN(100000), tfPassive));
            env.close();
            std::uint32_t const leg0OfferSeq = env.seq (leg0) - 1;

            env (offer (leg1, SUN(100000), USD(1000),   tfPassive));
            env.close();
            ...
            for (auto const& actor : t.actors)
            {
                ...
                env.require (balance (actor.acct, actor.sun));
...
{ 0, 0, 0, BTC( 5), { {"hye", 2, drops(3999999999950), BTC(5), USD(2000)}                                                      } }  // no xfer fee
    ...
    void testRequireAuth (FeatureBitset features)
    {
        ...
        env.fund (SUN(400000), gw, alice, bob);
        env.close();
        ...
        env (offer (alice, gwUSD(40), SUN(4000)));
        env.close();
        ...
        env (offer (bob, SUN(4000), gwUSD(40)));
        env.close();
    ...
    void testMissingAuth (FeatureBitset features)
    {
        ...
        // untouched.
        env (offer (alice, gwUSD(40), SUN(4000)), ter(tecNO_LINE));
        env.close();
        ...
        env (offer (alice, gwUSD(40), SUN(4000)), ter(tecNO_AUTH));
        env.close();
    ...
    {
        testcase("SUNNYCoinConnect Smoketest payment flow");
        using namespace jtx;
        ...
        // This test mimics the payment flow used in the SUNNYCoin Connect
        ...
        env.fund (SUN(100000), hotUS, coldUS, hotEU, coldEU, mm);
        env.close();

        // Cold wallets require trust but will sunnycoin by default
        for (auto const& cold : {coldUS, coldEU})
        {
            env(fset (cold, asfRequireAuth));
            env(fset (cold, asfDefaultSUNNYCoin));
        }
        env.close();

        // Each hot wallet trusts the related cold wallet for a large amount
        env (trust(hotUS, USD(10000000)), txflags (tfSetNoSUNNYCoin));
        env (trust(hotEU, EUR(10000000)), txflags (tfSetNoSUNNYCoin));
        // Market maker trusts both cold wallets for a large amount
        env (trust(mm, USD(10000000)), txflags (tfSetNoSUNNYCoin));
        env (trust(mm, EUR(10000000)), txflags (tfSetNoSUNNYCoin));
        env.close();
        ...
        // There should be a path available from hot US to cold EUR
        {
            ...
            Json::Value const jrr {env.rpc(
                "json", "sunnycoin_path_find", to_string(jvParams))[jss::result]};
    ...
    void testSelfAuth (FeatureBitset features)
    {
        ...
        env.fund (SUN(400000), gw, alice);
        env.close();

        // Test that gw can create an offer to buy gw's currency.
        env (offer (gw, gwUSD(40), SUN(4000)));
        env.close();
        std::uint32_t const gwOfferSeq = env.seq (gw) - 1;
        env.require (offers (gw, 1));
        ...
        env (offer (alice, SUN(4000), gwUSD(40)));
        env.close();
    ...
    void testAll(FeatureBitset features)
    {
        ...
        testSUNTinyPayment(features);
        testEnforceNoSUNNYCoin(features);
        ...
        testOfferCrossWithSUN(true, features);
        testOfferCrossWithSUN(false, features);
        ...
        testCrossCurrencyStartSUN(features);
        testCrossCurrencyEndSUN(features);
...
BEAST_DEFINE_TESTSUITE_PRIO (Offer, tx, sunnycoin, 4);
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO (Offer_manual, tx, sunnycoin, 20);
