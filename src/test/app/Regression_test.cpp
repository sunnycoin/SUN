...
struct Regression_test : public beast::unit_test::suite
{
    ...
    void testOffer1()
    {
        ...
        env(offer("alice", USD(10), SUN(10)), require(owners("alice", 1)));
        env(offer("alice", USD(20), SUN(10)), json(R"raw(
                { "OfferSequence" : 2 }
    ...
    {
        testcase("Account balance < fee destroys correct amount of SUN");
        using namespace jtx;
        Env env(*this);
        env.memoize("alice");
        ...
        auto const aliceSUN = 400;
        auto const aliceAmount = SUN(aliceSUN);
            ...
            env.app().timeKeeper().closeTime());
        {
            ...
            auto const result = sunnycoin::apply(env.app(),
        ...
        }
        {
            ...
            BEAST_EXPECT(balance == SUN(0));
        }
        expectedDrops -= aliceSUN * dropsPerSUN<int>::value;
        BEAST_EXPECT(next->info().drops == expectedDrops);
    }
...
BEAST_DEFINE_TESTSUITE(Regression,app,sunnycoin);
