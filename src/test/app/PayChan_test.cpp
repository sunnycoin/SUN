...
#include <sunnycoin/protocol/PayChan.h>
...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    {
        Serializer msg;
        serializePayChanAuthorization (msg, channel, authAmt.sun ());
    ...
    testSimple ()
    {
        ...
        env (create (alice, bob, SUN (1000), settleDelay, pk));
        auto const chan = channel (*env.current (), alice, bob);
        BEAST_EXPECT (channelBalance (*env.current (), chan) == SUN (0));
        BEAST_EXPECT (channelAmount (*env.current (), chan) == SUN (1000));

        {
            auto const preAlice = env.balance (alice);
            env (fund (alice, chan, SUN (1000)));
            auto const feeDrops = env.current ()->fees ().base;
            BEAST_EXPECT (env.balance (alice) == preAlice - SUN (1000) - feeDrops);
        }

        auto chanBal = channelBalance (*env.current (), chan);
        auto chanAmt = channelAmount (*env.current (), chan);
        BEAST_EXPECT (chanBal == SUN (0));
        BEAST_EXPECT (chanAmt == SUN (2000));

        {
            // bad amounts (non-sun, negative amounts)
        ...
        // invalid account
        env (create (alice, "noAccount", SUN (1000), settleDelay, pk),
            ter (tecNO_DST));
        // can't create channel to the same account
        env (create (alice, alice, SUN (1000), settleDelay, pk),
             ter (temDST_IS_SRC));
        // invalid channel
        env (fund (alice, channel (*env.current (), alice, "noAccount"), SUN (1000)),
            ter (tecNO_ENTRY));
        // not enough funds
        env (create (alice, bob, SUN (10000), settleDelay, pk),
            ter (tecUNFUNDED));

        {
            // No signature claim with bad amounts (negative and non-sun)
            auto const iou = USDA (100).value ();
            auto const negSUN = SUN (-100).value ();
            auto const posSUN = SUN (100).value ();
            env (claim (alice, chan, iou, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posSUN, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, iou, posSUN), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negSUN, negSUN), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posSUN, negSUN), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negSUN, posSUN), ter (temBAD_AMOUNT));
        }
        {
            // No signature claim more than authorized
            auto const delta = SUN (500);
            auto const reqBal = chanBal + delta;
            auto const authAmt = reqBal + SUN (-100);
        ...
        }
        {
            ...
            auto const preBob = env.balance (bob);
            STAmount const authAmt = chanBal + SUN (500);
        ...
        env (fund (bob, chan, SUN (1000)), ter (tecNO_PERMISSION));
        BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
        BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);

        {
            // Wrong signing key
            auto const sig =
                signClaimAuth (bob.pk (), bob.sk (), chan, SUN (1500));
            env (claim (bob, chan, SUN (1500).value (), SUN (1500).value (),
                     Slice (sig), bob.pk ()),
                ter (temBAD_SIGNER));
            BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
            BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);
    ...
    testCancelAfter ()
    {
        ...
        auto const carol = Account ("carol");
        {
            ...
            auto const channelFunds = SUN (1000);
            env (create (
    ...
    testExpiration ()
    {
        ...
        env (fund (alice, chan, SUN (1),
            ...
            ter (temBAD_EXPIRATION));
        BEAST_EXPECT (!channelExpiration (*env.current (), chan));
        env (fund (alice, chan, SUN (1), NetClock::time_point{minExpiration}));
        env.close (minExpiration);
        // Try to extend the expiration after the expiration has already passed
        env (fund (
            alice, chan, SUN (1), NetClock::time_point{minExpiration + 1000s}));
        BEAST_EXPECT (!channelExists (*env.current (), chan));
    }
    ...
    testDisallowSUN ()
    {
        ...
        auto const bob = Account ("bob");
        {
            // Create a channel where dst disallows SUN
            Env env (*this, supported_amendments() - featureDepositAuth);
            env.fund (SUN (10000), alice, bob);
            env (fset (bob, asfDisallowSUN));
            env (create (alice, bob, SUN (1000), 3600s, alice.pk()),
                ter (tecNO_TARGET));
            auto const chan = channel (*env.current (), alice, bob);
            BEAST_EXPECT (!channelExists (*env.current (), chan));
        }
        {
            // Create a channel where dst disallows SUN.  Ignore that flag,
        ...
        {
            // Claim to a channel where dst disallows SUN
            // (channel is created before disallow sun is set)
            Env env (*this, supported_amendments() - featureDepositAuth);
            env.fund (SUN (10000), alice, bob);
            env (create (alice, bob, SUN (1000), 3600s, alice.pk()));
            auto const chan = channel (*env.current (), alice, bob);
            ...
            env (fset (bob, asfDisallowSUN));
            auto const reqBal = SUN (500).value();
            env (claim (alice, chan, reqBal, reqBal), ter(tecNO_TARGET));
        }
        {
            // Claim to a channel where dst disallows SUN (channel is
            // created before disallow sun is set).  Ignore that flag
    ...
    testDepositAuth ()
    {
        ...
        auto USDA = alice["USD"];
        {
            ...
            env (claim (alice, chan,
                SUN (500).value(), SUN (500).value()), ter (tecNO_PERMISSION));
            env.close();
            ...
            auto const preBob = env.balance (bob);
            {
                auto const delta = SUN (500).value();
            ...
            }
            {
                ...
                auto const delta = SUN (600).value();
                auto const sig = signClaimAuth (pk, alice.sk (), chan, delta);
            ...
            }
            {
                ...
                auto const delta = SUN (800).value();
                ...
                env.close();
                BEAST_EXPECT (
                    env.balance (bob) == preBob + SUN (800) - (5 * baseFee));
            }
        }
    }
    ...
    testMalformedPK ()
    {
        ...
        auto jv = create (alice, bob, SUN (1000), settleDelay, pk);
...
BEAST_DEFINE_TESTSUITE (PayChan, app, sunnycoin);
}  // test
}  // sunnycoin
