...
class Discrepancy_test : public beast::unit_test::suite
{
    ...
    void
    testSUNDiscrepancy (FeatureBitset features)
    {
        testcase ("Discrepancy test : SUN Discrepancy");
        ...
        env.fund(SUN(2000), A1);
        env.fund(SUN(1000), A2, A6, A7);
        env.fund(SUN(5000), A3);
        env.fund(SUN(1000000), A4);
        env.fund(SUN(600000), A5);
        env.close();
        ...
        env(offer(A4, SUN(49147), A2["JPY"](34501)));
        env(offer(A5, A3["CNY"](3150), SUN(80086)));
        env(offer(A7, SUN(1233), A6["CNY"](25)));
        env.close();

        test::PathSet payPaths {
            test::Path {A2["JPY"], A2},
            test::Path {SUN, A2["JPY"], A2},
            test::Path {A6, SUN, A2["JPY"], A2} };
...
public:
    void run () override
    {
        ...
        testSUNDiscrepancy (sa - featureFlow - fix1373 - featureFlowCross);
        testSUNDiscrepancy (sa               - fix1373 - featureFlowCross);
        testSUNDiscrepancy (sa                         - featureFlowCross);
        testSUNDiscrepancy (sa);
    }
};

BEAST_DEFINE_TESTSUITE (Discrepancy, app, sunnycoin);
