...
bool getNoSUNNYCoinFlag (jtx::Env const& env,
    ...
    Currency const& cur)
{
    ...
    {
        auto const flag = (src.id() > dst.id()) ? lsfHighNoSUNNYCoin : lsfLowNoSUNNYCoin;
...
sunMinusFee (jtx::Env const& env, std::int64_t sunAmount)
{
    ...
    return drops (
        dropsPerSUN<std::int64_t>::value * sunAmount - feeDrops);
};
...
struct Flow_test : public beast::unit_test::suite
{
    void testDirectStep (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env.fund (SUN (10000), alice, bob);
            env (pay (alice, bob, SUN (100)));
            env.require (balance (bob, SUN (10000 + 100)));
            env.require (balance (alice, sunMinusFee (env, 10000 - 100)));
        }
        {
            // Partial payments
            Env env (*this, features);
        ...
        }
        {
            ...
            env.fund (SUN (10000), alice, bob, carol, dan);
        ...
        }
        {
            ...
            env (pay (alice, dan, USDC (5)), path (bob, carol),
                sendmax (USDA (6)), txflags (tfNoSUNNYCoinDirect));
        ...
        }
        {
            ...
            env.fund (SUN (10000), alice, bob, carol, dan, erin);
            ...
            env (pay (alice, erin, USDD (5)), path (carol, dan),
                path (bob, dan), txflags (tfNoSUNNYCoinDirect));
        ...
        for (auto bobDanQIn : {80, 100, 120})
            for (auto bobAliceQOut : {80, 100, 120})
            {
                ...
                env(pay(dan, carol, USDA(10)),
                    path(bob), sendmax(USDD(100)), txflags(tfNoSUNNYCoinDirect));
    ...
    void testBookStep (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (offer (bob, BTC (50), SUN (50)));
            env (offer (bob, SUN (50), USD (50)));

            env (pay (alice, carol, USD (50)), path (~SUN, ~USD),
                sendmax (BTC (50)));
            ...
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, SUN (50), USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, BTC (50), SUN (50)));
        }
        {
            // simple SUN -> USD through offer and sendmax
            Env env (*this, features);
            ...
            env.require (balance (bob, sunMinusFee (env, 10000 + 50)));
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, SUN (50), USD (50)));
        }
        {
            // simple USD -> SUN through offer and sendmax
            Env env (*this, features);
            ...
            env (offer (bob, USD (50), SUN (50)));

            env (pay (alice, carol, SUN (50)), path (~SUN),
                 sendmax (USD (50)));

            env.require (balance (alice, USD (0)));
            env.require (balance (bob, sunMinusFee (env, 10000 - 50)));
            env.require (balance (bob, USD (50)));
            env.require (balance (carol, SUN (10000 + 50)));
            BEAST_EXPECT(!isOffer (env, bob, USD (50), SUN (50)));
        }
        {
        ...
        }
        {
            ...
            env (pay (alice, carol, EUR (1)), path (~SUN, ~EUR),
                sendmax (USD (0.4)), txflags (tfNoSUNNYCoinDirect|tfPartialPayment));
    ...
    void testTransferRate (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (pay (alice, carol, USD (50)), path (~USD), sendmax (SUN (50)));
            env.require (
                balance (alice, sunMinusFee (env, 10000 - 50)),
...

        {
            ...
            env (pay (alice, carol, EUR (40)), path (~USD, ~EUR), sendmax (SUN (40)));
            env.require (
                balance (alice,  sunMinusFee (env, 10000 - 40)),
                balance (bob, USD (40)),
                balance (bob, EUR (0)),
        ...
        }
        {
            ...
            env (pay (alice, carol, EUR (50)), path (bob, gw, ~EUR),
                sendmax (USDA (60)), txflags (tfNoSUNNYCoinDirect));
            env.require (
                balance (bob, USD (-10)),
                balance (bob, USDA (60)),
                balance (dan, USD (50)),
...

        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, SUN (100), USD (100)));
            env (pay (alice, bob, USD (100)),
                 sendmax (SUN (100)));
            env.require (
                balance (alice, sunMinusFee(env, 10000-100)),
                balance (bob, USD (100)));
        }
        if (!features[featureOwnerPaysFee])
        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, SUN (125), USD (125)));
            env (pay (alice, bob, USD (100)),
                 sendmax (SUN (200)));
            env.require (
                balance (alice, sunMinusFee(env, 10000-125)),
                balance (bob, USD (100)));
        }
    }
    ...
    testFalseDry(FeatureBitset features)
    {
        ...
        env.fund (SUN(10000), alice, carol, gw);
        ...
        // Bob has _just_ slightly less than 50 sun available
        // If his owner count changes, he will have more liquidity.
        // This is one error case to test (when Flow is used).
        // Computing the incomming sun to the SUN/USD offer will require two
        // recursive calls to the EUR/SUN offer. The second call will return
        // tecPATH_DRY, but the entire path should not be marked as dry. This
        // is the second error case to test (when flowV1 is used).
        env (offer (bob, EUR (50), SUN (50)));
        env (offer (bob, SUN (50), USD (50)));

        env (pay (alice, carol, USD (1000000)), path (~SUN, ~USD),
            sendmax (EUR (500)),
    ...
    testLimitQuality ()
    {
        ...
        for (auto const& d : {-100 * timeDelta, +100 * timeDelta})
        {
            ...
            env (offer (bob, SUN (50), USD (50)));
            env (offer (bob, SUN (100), USD (50)));

            TER const expectedResult = closeTime < fix1141Time ()
                ? TER {tecPATH_DRY}
                : TER {tesSUCCESS};
            env (pay (alice, carol, USD (100)), path (~USD), sendmax (SUN (100)),
                txflags (tfNoSUNNYCoinDirect | tfPartialPayment | tfLimitQuality),
                ter (expectedResult));
    ...
    static SUNAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    void testSelfFundedSUNEndpoint (bool consumeOffer, FeatureBitset features)
    {
        // Test that the deferred credit table is not bypassed for
        // SUNEndpointSteps. If the account in the first step is sending SUN and
        // that account also owns an offer that receives SUN, it should not be
        // possible for that step to use the SUN received in the offer as part
        // of the payment.
        testcase("Self funded SUNEndpoint");
        ...
        env(offer(alice, SUN(50000), USD(10)));
        ...
        env(pay(alice, alice, toSend), path(~USD), sendmax(SUN(20000)),
            txflags(tfPartialPayment | tfNoSUNNYCoinDirect));
    }
    ...
    void testUnfundedOffer (bool withFix, FeatureBitset features)
    {
        ...
        using namespace jtx;
        {
            ...
            env.fund(SUN(100000), alice, bob, gw);
            ...
            env(pay(alice, bob, tinyAmt1), path(~USD),
                sendmax(drops(9000000000)), txflags(tfNoSUNNYCoinDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, SUN(0), USD(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, SUN(0), USD(0)));
        }
        {
            ...
            env(pay(alice, bob, drops(9000000000)), path(~SUN),
                sendmax(USD(1)), txflags(tfNoSUNNYCoinDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, USD(0), SUN(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, USD(0), SUN(0)));
        }
    }
    ...
    testReexecuteDirectStep(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(!getNoSUNNYCoinFlag(env, gw, alice, usdC));
        ...
        env(pay(alice, bob, SUN(10000)), path(~SUN), sendmax(USD(100)),
            txflags(tfPartialPayment | tfNoSUNNYCoinDirect));
    }
    ...
    testRIPD1443(bool withFix)
    {
        ...
        env.fund(SUN(100000000), alice, nosunnycoin(bob), carol, gw);
        env.trust(gw["USD"](10000), alice, carol);
        env(trust(bob, gw["USD"](10000), tfSetNoSUNNYCoin));
        env.trust(gw["USD"](10000), bob);
        env.close();

        // set no sunnycoin between bob and the gateway

        env(pay(gw, alice, gw["USD"](1000)));
        env.close();

        env(offer(alice, bob["USD"](1000), SUN(1)));
        env.close();

        env(pay(alice, alice, SUN(1)), path(gw, bob, ~SUN),
            sendmax(gw["USD"](1000)), txflags(tfNoSUNNYCoinDirect),
        ...
        env(offer(alice, SUN(1000), bob["USD"](1000)));
        env.close();

        env(pay (carol, carol, gw["USD"](1000)), path(~bob["USD"], gw),
            sendmax(SUN(100000)), txflags(tfNoSUNNYCoinDirect),
            ter(withFix ? TER {tecPATH_DRY} : TER {tesSUCCESS}));
        env.close();
    ...
    testRIPD1449(bool withFix)
    {
        ...
        // pay alice -> sun -> USD/bob -> bob -> gw -> alice
        // set no sunnycoin on bob's side of the bob/gw trust line
        ...
        env.fund(SUN(100000000), alice, bob, carol, gw);
        env.trust(USD(10000), alice, carol);
        env(trust(bob, USD(10000), tfSetNoSUNNYCoin));
        ...
        env(offer(carol, SUN(1), bob["USD"](1000)));
        env.close();

        env(pay(alice, alice, USD(1000)), path(~bob["USD"], bob, gw),
            sendmax(SUN(1)), txflags(tfNoSUNNYCoinDirect),
            ter(withFix ? TER {tecPATH_DRY} : TER {tesSUCCESS}));
        env.close();
    }
    ...
    testSelfPayLowQualityOffer (FeatureBitset features)
    {
        ...
        // SUN than the offering account held.  This unit test reproduces
        // that failing case.
    ...
    void testWithFeats(FeatureBitset features)
    {
        ...
        testSelfFundedSUNEndpoint(false, features);
        testSelfFundedSUNEndpoint(true, features);
...
BEAST_DEFINE_TESTSUITE_PRIO(Flow,app,sunnycoin,2);
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(Flow_manual,app,sunnycoin,4);
