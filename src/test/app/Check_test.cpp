...
class Check_test : public beast::unit_test::suite
{
    ...
    void testEnabled()
    {
        ...
        Account const alice {"alice"};
        {
            ...
            env.fund (SUN(1000), alice);

            uint256 const checkId {
                getCheckIndex (env.master, env.seq (env.master))};
            env (check::create (env.master, alice, SUN(100)),
                ter(temDISABLED));
            env.close();

            env (check::cash (alice, checkId, SUN(100)),
                ter (temDISABLED));
            env.close();
        ...
        }
        {
            ...
            env (check::cash (alice, checkId1, SUN(100)));
            env.close();
    ...
    void testCreateValid()
    {
        ...
        STAmount const startBalance {SUN(1000).value()};
            ...
            [&env, &USD, this] (Account const& from, Account const& to)
        {
            ...
            env (check::create (from, to, SUN(2000)));
            env.close();
    ...
    void testCreateInvalid()
    {
        ...
        // Check to self.
        env (check::create (alice, alice, SUN(10)), ter (temREDUNDANT));
        env.close();
    ...
    void testCashSUN()
    {
        // Explore many of the valid ways to cash a check for SUN.
        testcase ("Cash SUN");
        ...
        env.fund (startBalance, alice, bob);
        {
            ...
            env (check::cash (bob, chkId, SUN(10)));
            env.close();
            env.require (balance (alice,
                startBalance - SUN(10) - drops (baseFeeDrops)));
            ...
            env (pay (env.master, alice, SUN(10) + drops (baseFeeDrops)));
            env (pay (bob, env.master,   SUN(10) - drops (baseFeeDrops * 2)));
            env.close();
    ...
    void testCashIOU ()
    {
        ...
        IOU const USD {gw["USD"]};
        {
            ...
            env.fund (SUN(1000), gw, alice, bob);
    ...
    void testCashInvalid()
    {
        ...
        env.fund (SUN(1000), gw, alice, bob, zoe);
        ...
        env (check::create (alice, bob, SUN(10)), expiration (env.now() + 1s));
        env.close();
        ...
        // Same set of failing cases for both IOU and SUN check cashing.
        ...
        failingCases (chkIdX, SUN(10));
        failingCases (chkIdU, USD(20));

        // Verify that those two checks really were cashable.
        env (check::cash (bob, chkIdU, USD(20)));
        env.close();
        env (check::cash (bob, chkIdX, check::DeliverMin (SUN(10))));
        verifyDeliveredAmount (env, SUN(10));

        // Try to cash an expired check.
        env (check::cash (bob, chkIdExp, SUN(10)), ter (tecEXPIRED));
        env.close();
    ...
    void testCancelValid()
    {
        ...
        env (check::create (alice, bob, SUN(10)), expiration (env.now()+600s));
        env.close();
    ...
    void testFix1623Enable ()
    {
        ...
        auto testEnable = [this] (FeatureBitset const& features, bool hasFields)
        {
            ...
            uint256 const chkId {getCheckIndex (alice, env.seq (alice))};
            env (check::create (alice, bob, SUN(200)));
            env.close();

            env (check::cash (bob, chkId, check::DeliverMin (SUN(100))));
...
BEAST_DEFINE_TESTSUITE (Check, tx, sunnycoin);
