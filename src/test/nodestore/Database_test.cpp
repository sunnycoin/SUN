...
public:
        ...
        if (type == "memory")
        {
            // Earliest ledger sequence tests
            {
                ...
                std::unique_ptr<Database> db =
                    Manager::instance().make_Database(
                        "test", scheduler, 2, parent, nodeParams, j);
                BEAST_EXPECT(db->earliestSeq() == SUN_LEDGER_EARLIEST_SEQ);
            }
            ...
            try
            {
                ...
                nodeParams.set("earliest_seq",
                    std::to_string(SUN_LEDGER_EARLIEST_SEQ));
    ...
    void run () override
    {
        ...
        #if SUNNYCOIN_ENABLE_SQLITE_BACKEND_TESTS
...
BEAST_DEFINE_TESTSUITE(Database,NodeStore,sunnycoin);
