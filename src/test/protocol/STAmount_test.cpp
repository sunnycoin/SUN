...
#include <sunnycoin/basics/random.h>
...
public:
    ...
    {
        {
            ...
            Issue const sun (sunIssue ());

            // fractional SUN (i.e. drops)
            testSetValue ("1", sun);
            testSetValue ("22", sun);
            testSetValue ("333", sun);
            testSetValue ("4444", sun);
            testSetValue ("55555", sun);
            testSetValue ("666666", sun);

            // 1 SUN up to 100 billion, in powers of 10 (in drops)
            testSetValue ("1000000", sun);
            testSetValue ("10000000", sun);
            testSetValue ("100000000", sun);
            testSetValue ("1000000000", sun);
            testSetValue ("10000000000", sun);
            testSetValue ("100000000000", sun);
            testSetValue ("1000000000000", sun);
            testSetValue ("10000000000000", sun);
            testSetValue ("100000000000000", sun);
            testSetValue ("1000000000000000", sun);
            testSetValue ("10000000000000000", sun);
            testSetValue ("100000000000000000", sun);

            // Invalid native values:
            testSetValue ("1.1", sun, false);
            testSetValue ("100000000000000001", sun, false);
            testSetValue ("1000000000000000000", sun, false);
        }
    ...
    void testNativeCurrency ()
    {
        ...
        unexpected (to_string (Currency ()) != "SUN", "cHC(SUN)");
        Currency c;
    ...
    void testCustomCurrency ()
    {
        ...
        unexpected (multiply (STAmount (noIssue(), 20), STAmount (3), sunIssue ()).getText () != "60",
            "STAmount multiply fail 2");
        unexpected (multiply (STAmount (20), STAmount (3), noIssue()).getText () != "60",
            "STAmount multiply fail 3");
        unexpected (multiply (STAmount (20), STAmount (3), sunIssue ()).getText () != "60",
        ...
        unexpected (divide (STAmount (noIssue(), 60), STAmount (3), sunIssue ()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), noIssue()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), sunIssue ()).getText () != "20",
            "STAmount divide fail");
    ...
    void testUnderflow ()
    {
        ...
        bigDsmall = divide (smallValue, bigValue, sunIssue ());

        BEAST_EXPECT(bigDsmall == beast::zero);

        bigDsmall = divide (smallValue, bigNative, sunIssue ());
    ...
    void testRounding ()
    {
        ...
        STAmount dripTest1 = mulRound (twoThird2, two, sunIssue (), false);
        STAmount dripTest2 = multiply (twoThird2, two, sunIssue ());
        STAmount dripTest3 = mulRound (twoThird2, two, sunIssue (), true);
        log << dripTest1;
        log << dripTest2;
        log << dripTest3;
#endif
    ...
    testConvertSUN ()
    {
        testcase ("STAmount to SUNAmount conversions");
        ...
        {
            auto const t = amountFromString (sun, std::to_string (drops));
            auto const s = t.sun ();
            BEAST_EXPECT(s.drops() == drops);
            BEAST_EXPECT(t == STAmount (SUNAmount (drops)));
            BEAST_EXPECT(s == SUNAmount (drops));
        }

        try
        {
            auto const t = amountFromString (usd, "136500");
            fail (to_string (t.sun ()));
        }
    ...
    testConvertIOU ()
    {
        ...
        try
        {
            auto const t = amountFromString (sun, "136500");
...
BEAST_DEFINE_TESTSUITE(STAmount,sunnycoin_data,sunnycoin);
