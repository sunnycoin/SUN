...
void
Env::fund (bool setDefaultSUNNYCoin,
    STAmount const& amount,
        Account const& account)
{
    memoize(account);
    if (setDefaultSUNNYCoin)
    {
        ...
        apply(pay(master, account, amount +
            drops(current()->fees().base)),
                jtx::seq(jtx::autofill),
                    fee(jtx::autofill),
                        sig(jtx::autofill));
        apply(fset(account, asfDefaultSUNNYCoin),
            jtx::seq(jtx::autofill),
                fee(jtx::autofill),
                    sig(jtx::autofill));
        require(flags(account, asfDefaultSUNNYCoin));
    }
    else
    {
        apply(pay(master, account, amount),
            jtx::seq(jtx::autofill),
                fee(jtx::autofill),
                    sig(jtx::autofill));
        require(nflags(account, asfDefaultSUNNYCoin));
    }
