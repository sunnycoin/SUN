...
#ifndef SUNNYCOIN_TEST_JTX_ENV_H_INCLUDED
#define SUNNYCOIN_TEST_JTX_ENV_H_INCLUDED
...
#include <sunnycoin/protocol/STObject.h>
#include <sunnycoin/protocol/STTx.h>
...
/** Designate accounts as no-sunnycoin in Env::fund */
template <class... Args>
std::array<Account, 1 + sizeof...(Args)>
nosunnycoin (Account const& account, Args const&... args)
{
    return {{account, args...}};
}
...
supported_amendments()
{
    static const FeatureBitset ids = []{
        auto const& sa = sunnycoin::detail::supportedAmendments();
...
public:
    ...
    /** Returns the current SUNNYCoin Network Time
    ...
    */
    ...
    /** Returns the SUN balance on an account.
...
public:
    ...
    /** Create a new account with some SUN.
        ...
        Effects:
            The asfDefaultSUNNYCoin on the account is set,
            and the sequence number is incremented, unless
            the account is wrapped with a call to nosunnycoin.

            The account's SUN balance is set to amount.

            Generates a test that the balance is set.

        @param amount The amount of SUN to transfer to
                      each account.

        @param args A heterogeneous list of accounts to fund
                    or calls to nosunnycoin with lists of accounts
                    to fund.
    */
