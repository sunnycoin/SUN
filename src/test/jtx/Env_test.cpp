...
#include <sunnycoin/beast/hash/uhash.h>
...
public:
    ...
    testAmount()
    {
        ...
        BEAST_EXPECT(to_string(SUN(5)) == "5 SUN");
        BEAST_EXPECT(to_string(SUN(.80)) == "0.8 SUN");
        BEAST_EXPECT(to_string(SUN(.005)) == "5000 drops");
        BEAST_EXPECT(to_string(SUN(0.1)) == "0.1 SUN");
        BEAST_EXPECT(to_string(SUN(10000)) == "10000 SUN");
        BEAST_EXPECT(to_string(drops(10)) == "10 drops");
        BEAST_EXPECT(to_string(drops(123400000)) == "123.4 SUN");
        BEAST_EXPECT(to_string(SUN(-5)) == "-5 SUN");
        BEAST_EXPECT(to_string(SUN(-.99)) == "-0.99 SUN");
        BEAST_EXPECT(to_string(SUN(-.005)) == "-5000 drops");
        BEAST_EXPECT(to_string(SUN(-0.1)) == "-0.1 SUN");
        BEAST_EXPECT(to_string(drops(-10)) == "-10 drops");
        BEAST_EXPECT(to_string(drops(-123400000)) == "-123.4 SUN");

        BEAST_EXPECT(SUN(1) == drops(1000000));
        BEAST_EXPECT(SUN(1) == STAmount(1000000));
        BEAST_EXPECT(STAmount(1000000) == SUN(1));
    ...
    testEnv()
    {
        ...
        {
            Env env(*this);
            env(pay("alice", "bob", SUN(1000)), seq(1), fee(10), sig("alice"), ter(terNO_ACCOUNT));
        }
        ...
        // fund
        {
            ...
            env.fund(n, "dave", nosunnycoin("eric"));
            env.fund(n, "fred", nosunnycoin("gary", "hank"));
            env.fund(n, nosunnycoin("irene"));
            env.fund(n, nosunnycoin("jim"), "karen");
            env.fund(n, nosunnycoin("lisa", "mary"));

            // flags
            env.fund(n, nosunnycoin("xavier"));
            env.require(nflags("xavier", asfDefaultSUNNYCoin));
            env.fund(n, "yana");
            env.require(flags("yana", asfDefaultSUNNYCoin));
        }
        ...
        {
            Env env(*this);
            env.fund(n, nosunnycoin("alice", gw));
    ...
    testRequire()
    {
        ...
        env.require(balance("alice", SUN(none)));
        env.fund(SUN(10000), "alice", gw);
        env.require(balance("alice", USD(none)));
        env.trust(USD(100), "alice");
        env.require(balance("alice", SUN(10000))); // fee refunded
    ...
    testPayments()
    {
        ...
        env.require(balance("alice", SUN(10000)));
        env.require(balance("bob", SUN(10000)));
        env.require(balance("carol", SUN(10000)));
        env.require(balance(gw, SUN(10000)));

        env(pay(env.master, "alice", SUN(1000)), fee(none),     ter(temMALFORMED));
        env(pay(env.master, "alice", SUN(1000)), fee(1),        ter(telINSUF_FEE_P));
        env(pay(env.master, "alice", SUN(1000)), seq(none),     ter(temMALFORMED));
        env(pay(env.master, "alice", SUN(1000)), seq(20),       ter(terPRE_SEQ));
        env(pay(env.master, "alice", SUN(1000)), sig(none),     ter(temMALFORMED));
        env(pay(env.master, "alice", SUN(1000)), sig("bob"),    ter(tefBAD_AUTH_MASTER));

        env(pay(env.master, "dilbert", SUN(1000)), sig(env.master));
        ...
        env(offer("carol", SUN(50), USD(50)), require(owners("carol", 2)));
        env(pay("alice", "bob", any(USD(10))),                  ter(tecPATH_DRY));
        env(pay("alice", "bob", any(USD(10))),
            paths(SUN), sendmax(SUN(10)),                       ter(tecPATH_PARTIAL));
        env(pay("alice", "bob", any(USD(10))), paths(SUN),
            sendmax(SUN(20)));
...
BEAST_DEFINE_TESTSUITE(Env,app,sunnycoin);
