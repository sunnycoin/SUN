...
#include <sunnycoin/protocol/AmountConversions.h>
...
class PaymentSandbox_test : public beast::unit_test::suite
{
    ...
    */
    void testSelfFunding (FeatureBitset features)
    {
        ...
        env.fund (SUN (10000), snd, rcv, gw1, gw2);
    ...
    void testSubtractCredits (FeatureBitset features)
    {
        ...
        env.fund (SUN (10000), alice, gw1, gw2);
        ...
        {
            // sunnycoinCredit, no deferredCredits
            ApplyViewImpl av (&*env.current(), tapNONE);
            ...
            sunnycoinCredit (av, gw1, alice, toCredit, true, j);
            BEAST_EXPECT(accountHolds (av, alice, iss.currency, iss.account,
                        fhIGNORE_FREEZE, j) ==
                    startingAmount + toCredit);

            sunnycoinCredit (av, alice, gw1, toDebit, true, j);
            BEAST_EXPECT(accountHolds (av, alice, iss.currency, iss.account,
                        fhIGNORE_FREEZE, j) ==
                    startingAmount + toCredit - toDebit);
        }
        ...
        {
            // sunnycoinCredit, w/ deferredCredits
            ApplyViewImpl av (&*env.current(), tapNONE);
            PaymentSandbox pv (&av);
            ...
            sunnycoinCredit (pv, gw1, alice, toCredit, true, j);
            BEAST_EXPECT(accountHolds (pv, alice, iss.currency, iss.account,
                        fhIGNORE_FREEZE, j) ==
                    startingAmount);
        }
    ...
    void testReserve(FeatureBitset features)
    {
        ...
        auto accountFundsSUN = [&dj](
            ReadView const& view, AccountID const& id) -> SUNAmount
        {
            return toAmount<SUNAmount> (accountHolds (
                view, id, sunCurrency (), sunAccount (), fhZERO_IF_FROZEN, dj));
        };

        auto reserve = [](jtx::Env& env, std::uint32_t count) -> SUNAmount
        {
        ...
        PaymentSandbox sb (&av);
        {
            ...
            accountSend (sb, sunAccount (), alice, SUN(100), dj);
            accountSend (sb, alice, sunAccount (), SUN(100), dj);
            BEAST_EXPECT(accountFundsSUN (sb, alice) == beast::zero);
        }
    }
...
BEAST_DEFINE_TESTSUITE (PaymentSandbox, ledger, sunnycoin);
