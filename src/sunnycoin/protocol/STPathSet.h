...
#ifndef SUNNYCOIN_PROTOCOL_STPATHSET_H_INCLUDED
#define SUNNYCOIN_PROTOCOL_STPATHSET_H_INCLUDED
...
public:
    ...
    STPathElement (
        ...
        , mIssuerID (issuer), is_offer_ (isSUN(mAccountID))
    {
        if (!is_offer_)
            mType |= typeAccount;

        if (forceCurrency || !isSUN(currency))
            mType |= typeCurrency;
