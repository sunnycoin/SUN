...
#ifndef SUNNYCOIN_PROTOCOL_BOOK_H_INCLUDED
#define SUNNYCOIN_PROTOCOL_BOOK_H_INCLUDED
...
template <>
struct hash <sunnycoin::Issue>
    : private boost::base_from_member <std::hash <sunnycoin::Currency>, 0>
    , private boost::base_from_member <std::hash <sunnycoin::AccountID>, 1>
{
private:
    using currency_hash_type = boost::base_from_member <
        std::hash <sunnycoin::Currency>, 0>;
    using issuer_hash_type = boost::base_from_member <
        std::hash <sunnycoin::AccountID>, 1>;

public:
    explicit hash() = default;

    using value_type = std::size_t;
    using argument_type = sunnycoin::Issue;

    value_type operator() (argument_type const& value) const
    {
        value_type result (currency_hash_type::member (value.currency));
        if (!isSUN (value.currency))
...
template <>
struct hash <sunnycoin::Book>
{
private:
    using hasher = std::hash <sunnycoin::Issue>;
...
public:
    ...
    using value_type = std::size_t;
    using argument_type = sunnycoin::Book;
...
template <>
struct hash <sunnycoin::Issue>
    : std::hash <sunnycoin::Issue>
{
    explicit hash() = default;

    using Base = std::hash <sunnycoin::Issue>;
...
template <>
struct hash <sunnycoin::Book>
    : std::hash <sunnycoin::Book>
{
    explicit hash() = default;

    using Base = std::hash <sunnycoin::Book>;
    // VFALCO NOTE broken in vs2012
    //using Base::Base; // inherit ctors
};
