...
{
    return getSUNNYCoinStateIndex (a, issue.account, issue.currency);
}
...
{
    return { ltSUNNYCOIN_STATE,
        getSUNNYCoinStateIndex(id0, id1, currency) };
}
...
{
    return { ltSUNNYCOIN_STATE,
        getSUNNYCoinStateIndex(id, issue) };
}
...
payChan (AccountID const& source, AccountID const& dst, std::uint32_t seq)
{
    sha512_half_hasher h;
    using beast::hash_append;
    hash_append(h, std::uint16_t(spaceSUNUChannel));
    hash_append(h, source);
    hash_append(h, dst);
    hash_append(h, seq);
