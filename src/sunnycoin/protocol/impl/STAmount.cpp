    ...
    : STBase(name)
{
    ...
    if (isSUN (issue.currency))
        Throw<std::runtime_error> ("invalid native currency");

    issue.account.copyFrom (sit.get160 ());

    if (isSUN (issue.account))
...
STAmount::STAmount (SUNAmount const& amount)
    : mOffset (0)
    , mIsNative (true)
    , mIsNegative (amount < beast::zero)
{
...
//
//------------------------------------------------------------------------------
SUNAmount STAmount::sun () const
{
    if (!mIsNative)
        Throw<std::logic_error> ("Cannot return non-native STAmount as SUNAmount");
...
amountFromString (Issue const& issue, std::string const& amount)
{
    ...
    // Can't specify SUN using fractional representation
    if (isSUN(issue) && match[3].matched)
        Throw<std::runtime_error> ("SUN must be specified in integral drops.");
...
amountFromJson (SField const& name, Json::Value const& v)
{
    ...
    if (v.isNull())
    {
        Throw<std::runtime_error> ("SUN may not be specified with a null Json value");
    }
    else if (v.isObject())
    {
        value       = v[jss::value];
    ...
    {
        if (v.isObjectOrNull ())
            Throw<std::runtime_error> ("SUN may not be specified as an object");
        issue = sunIssue ();
    }
    else
    {
...
multiply (STAmount const& v1, STAmount const& v2, Issue const& issue)
{
    ...
    if (v1.native() && v2.native() && isSUN (issue))
    {
    ...
    bool roundUp)
{
    ...
    bool const sun = isSUN (issue);

    if (v1.native() && v2.native() && sun)
    {
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (sun, amount, offset);
    ...
    {
        if (sun && *stAmountCalcSwitchover2)
        {
    ...
    Issue const& issue, bool roundUp)
{
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (isSUN (issue), amount, offset);

    STAmount result (issue, amount, offset, resultNegative);
    // Control when bugfixes that require switchover dates are enabled
    if (roundUp && !resultNegative && !result && *stAmountCalcSwitchover)
    {
        if (isSUN(issue) && *stAmountCalcSwitchover2)
        {
            // return the smallest value above zero
            amount = 1;
            offset = 0;
