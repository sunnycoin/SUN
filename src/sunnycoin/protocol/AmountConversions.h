...
#ifndef SUNNYCOIN_PROTOCOL_AMOUNTCONVERSION_H_INCLUDED
#define SUNNYCOIN_PROTOCOL_AMOUNTCONVERSION_H_INCLUDED
...
inline
STAmount
toSTAmount (SUNAmount const& sun)
{
    bool const isNeg = sun.signum() < 0;
    std::uint64_t const umant = isNeg ? - sun.drops () : sun.drops ();
    return STAmount (umant, isNeg);
}

inline
STAmount
toSTAmount (SUNAmount const& sun, Issue const& iss)
{
    assert (isSUN(iss.account) && isSUN(iss.currency));
    return toSTAmount (sun);
}
...
inline
SUNAmount
toAmount<SUNAmount> (STAmount const& amt)
{
    assert (amt.mantissa () < std::numeric_limits<std::int64_t>::max ());
    bool const isNeg = amt.negative ();
    std::int64_t const sMant =
