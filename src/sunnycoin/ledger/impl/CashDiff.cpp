...
#include <sunnycoin/ledger/CashDiff.h>
#include <sunnycoin/ledger/detail/ApplyStateTable.h>
...
struct CashSummary
{
    ...
    std::vector<std::pair<
        AccountID, SUNAmount>> sunChanges;
    ...
    void sort()
    {
        std::sort (sunChanges.begin(), sunChanges.end());
    ...
    std::shared_ptr<SLE const> const& after)
{
    if (isDelete)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            result.sunChanges.push_back (
                std::make_pair (prev[sfAccount], SUNAmount {0}));
            return true;
    ...
    else
    {
        ...
        case ltACCOUNT_ROOT:
        {
            auto const curJrp = cur[sfBalance].sun();
            if (!before || (*before)[sfBalance].sun() != curJrp)
                result.sunChanges.push_back (
...
{
private:
    // Note differences in destroyed SUN between two ApplyStateTables.
    struct DropsGone
    {
        SUNAmount lhs;
        SUNAmount rhs;
...
countKeys (detail::CashSummary const& lhs, detail::CashSummary const& rhs)
{
    ...
    addIn (countKeys(lhs.sunChanges,     rhs.sunChanges));
...
int CashDiff::Impl::sunRoundToZero () const
{
    // The case has one OfferChange that is present on both lhs_ and rhs_.
    // That OfferChange should have SUN for TakerGets.  There should be a 1
    ...
    // The side with the smaller SUN balance in the OfferChange should have
    // two SUN differences.  The other side should have no SUN differences.
    if (smaller.sunChanges.size() != 2)
        return 0;
    if (! bigger.sunChanges.empty())
        return 0;
...
bool CashDiff::Impl::rmDust ()
{
    ...
    // amount differences: sunChanges, trustChanges, offerChanges, and
    // offerDeletions.  Rifle through those containers and remove any
    // entries that are _almost_ the same between lhs and rhs.

    // sunChanges.  We call a difference of 2 drops or less dust.
    removedDust |= rmVecDust (lhsDiffs_.sunChanges, rhsDiffs_.sunChanges,
        [](SUNAmount const& lhs, SUNAmount const& rhs)
        {
        ...
        CashFilter rhsFilter, detail::ApplyStateTable const& rhs)
{
    ...
    // sunChanges:
    setDiff (lhsDiffs.sunChanges, rhsDiffs.sunChanges, lhsDiffs_.sunChanges);
    setDiff (rhsDiffs.sunChanges, lhsDiffs.sunChanges, rhsDiffs_.sunChanges);
...
int CashDiff::sunRoundToZero() const
{
    return impl_->sunRoundToZero();
}
...
bool diffIsDust (STAmount const& v1, STAmount const& v2, std::uint8_t e10)
{
    ...
    // Handling SUN is different from IOU.
    if (v1.native())
    {
