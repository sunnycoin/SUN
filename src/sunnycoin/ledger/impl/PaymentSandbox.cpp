        ...
        STAmount const& amount) const
{
    ...
    */
    ...
    if (isSUN(issuer) && adjustedAmt < beast::zero)
        // A calculated negative SUN balance is not an error case. Consider a
        // payment snippet that credits a large SUN amount and then debits the
...
PaymentSandbox::balanceChanges (ReadView const& view) const
{
    ...
    // get the change in SUN balance, Account == root, issuer == root, currency == SUN
...
SUNAmount PaymentSandbox::sunDestroyed () const
{
