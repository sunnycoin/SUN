...
#include <sunnycoin/basics/chrono.h>
#include <sunnycoin/ledger/BookDirs.h>
...
#include <sunnycoin/protocol/st.h>
#include <sunnycoin/protocol/Protocol.h>
              ...
              beast::Journal j)
{
    ...
    if (isSUN(currency))
    {
        return {sunLiquid (view, account, 0, j)};
    }
...
SUNAmount
sunLiquid (ReadView const& view, AccountID const& id,
    std::int32_t ownerCountAdj, beast::Journal j)
{
    ...
    if (fix1141 (view.info ().parentCloseTime))
    {
        ...
        auto const balance = view.balanceHook(id, sunAccount(), fullBalance);
    ...
    {
        // pre-switchover
        // SUN: return balance minus reserve
        ...
        return view.balanceHook(id, sunAccount(), amount).sun();
    }
}
...
trustCreate (ApplyView& view,
    ...
    uint256 const&  uIndex,             // --> sunnycoin state entry
    SLE::ref        sleAccount,         // --> the account being set.
    const bool      bAuth,              // --> authorize account.
    const bool      bNoSUNNYCoin,          // --> others cannot sunnycoin through
    ...
    beast::Journal j)
{
    ...
    auto const sleSUNNYCoinState = std::make_shared<SLE>(
        ltSUNNYCOIN_STATE, uIndex);
    view.insert (sleSUNNYCoinState);

    auto lowNode = dirAdd (view, keylet::ownerDir (uLowAccountID),
        sleSUNNYCoinState->key(), false, describeOwnerDir (uLowAccountID), j);

    if (!lowNode)
        return tecDIR_FULL;

    auto highNode = dirAdd (view, keylet::ownerDir (uHighAccountID),
        sleSUNNYCoinState->key(), false, describeOwnerDir (uHighAccountID), j);
    ...
    sleSUNNYCoinState->setFieldU64 (sfLowNode, *lowNode);
    sleSUNNYCoinState->setFieldU64 (sfHighNode, *highNode);

    sleSUNNYCoinState->setFieldAmount (
    ...
    if (bNoSUNNYCoin)
    {
        uFlags |= (bSetHigh ? lsfHighNoSUNNYCoin : lsfLowNoSUNNYCoin);
    }
    if (bFreeze)
    {
    ...
    if ((slePeer->getFlags() & lsfDefaultSUNNYCoin) == 0)
    {
        // The other side's default is no rippling
        uFlags |= (bSetHigh ? lsfLowNoSUNNYCoin : lsfHighNoSUNNYCoin);
    }

    sleSUNNYCoinState->setFieldU32 (sfFlags, uFlags);
    adjustOwnerCount(view, sleAccount, 1, j);

    // ONLY: Create sunnycoin balance.
    sleSUNNYCoinState->setFieldAmount (sfBalance, bSetHigh ? -saBalance : saBalance);
...
trustDelete (ApplyView& view,
    std::shared_ptr<SLE> const& sleSUNNYCoinState,
        AccountID const& uLowAccountID,
            AccountID const& uHighAccountID,
                 beast::Journal j)
{
    // Detect legacy dirs.
    std::uint64_t uLowNode    = sleSUNNYCoinState->getFieldU64 (sfLowNode);
    std::uint64_t uHighNode   = sleSUNNYCoinState->getFieldU64 (sfHighNode);

    JLOG (j.trace())
        << "trustDelete: Deleting sunnycoin line: low";

    if (! view.dirRemove(
            keylet::ownerDir(uLowAccountID),
            uLowNode,
            sleSUNNYCoinState->key(),
            false))
    {
    ...
    JLOG (j.trace())
            << "trustDelete: Deleting sunnycoin line: high";
    ...
    JLOG (j.trace()) << "trustDelete: Deleting sunnycoin line: state";
    view.erase(sleSUNNYCoinState);
...
TER
sunnycoinCredit (ApplyView& view,
    ...
    beast::Journal j)
{
    ...
    uint256 uIndex = getSUNNYCoinStateIndex (
        uSenderID, uReceiverID, saAmount.getCurrency ());
    auto sleSUNNYCoinState  = view.peek (keylet::line(uIndex));

    TER terResult;

    assert (!isSUN (uSenderID) && uSenderID != noAccount());
    assert (!isSUN (uReceiverID) && uReceiverID != noAccount());

    if (!sleSUNNYCoinState)
    {
        ...
        bool noSUNNYCoin = (sleAccount->getFlags() & lsfDefaultSUNNYCoin) == 0;
    ...
    {
        STAmount saBalance   = sleSUNNYCoinState->getFieldAmount (sfBalance);
        ...
        std::uint32_t const uFlags (sleSUNNYCoinState->getFieldU32 (sfFlags));
        bool bDelete = false;
        ...
        if (saBefore > beast::zero
            ...
            && static_cast <bool> (uFlags & (!bSenderHigh ? lsfLowNoSUNNYCoin : lsfHighNoSUNNYCoin)) !=
               static_cast <bool> (view.read (keylet::account(uSenderID))->getFlags() & lsfDefaultSUNNYCoin)
            && !(uFlags & (!bSenderHigh ? lsfLowFreeze : lsfHighFreeze))
            && !sleSUNNYCoinState->getFieldAmount (
                !bSenderHigh ? sfLowLimit : sfHighLimit)
            // Sender trust limit is 0.
            && !sleSUNNYCoinState->getFieldU32 (
        ...
        sleSUNNYCoinState->setFieldAmount (sfBalance, saBalance);
        // ONLY: Adjust sunnycoin balance.
...
static
STAmount
sunnycoinTransferFee (ReadView const& view,
    ...
    beast::Journal j)
{
    if (from != issuer && to != issuer)
    {
        ...
        if (parityRate != rate)
        {
            ...
            JLOG (j.debug()) << "sunnycoinTransferFee:" <<
                " amount=" << amount.getFullText () <<
                " fee=" << fee.getFullText ();
...
static
TER
sunnycoinSend (ApplyView& view,
    AccountID const& uSenderID, AccountID const& uReceiverID,
    STAmount const& saAmount, STAmount& saActual, beast::Journal j)
{
    auto const issuer   = saAmount.getIssuer ();

    assert (!isSUN (uSenderID) && !isSUN (uReceiverID));
    assert (uSenderID != uReceiverID);

    if (uSenderID == issuer || uReceiverID == issuer || issuer == noAccount())
    {
        // Direct send: redeeming IOUs and/or sending own IOUs.
        sunnycoinCredit (view, uSenderID, uReceiverID, saAmount, false, j);
        saActual = saAmount;
        return tesSUCCESS;
    }
    ...
    if (!fix1141 (view.info ().parentCloseTime))
    {
        STAmount const saTransitFee = sunnycoinTransferFee (
    ...
    JLOG (j.debug()) << "sunnycoinSend> " <<
        to_string (uSenderID) <<
        " - > " << to_string (uReceiverID) <<
        " : deliver=" << saAmount.getFullText () <<
        " cost=" << saActual.getFullText ();

    TER terResult   = sunnycoinCredit (view, issuer, uReceiverID, saAmount, true, j);

    if (tesSUCCESS == terResult)
        terResult = sunnycoinCredit (view, uSenderID, issuer, saActual, true, j);
    ...
    STAmount const& saAmount, beast::Journal j)
{
    ...
    if (!saAmount.native ())
    {
        ...
        return sunnycoinSend (view, uSenderID, uReceiverID, saAmount, saActual, j);
    }
    ...
    /* SUN send which does not check reserve and can do pure adjustment.
    ...
    if (sender)
    {
        ...
        else
        {
            ...
            if (fv2Switch)
                view.creditHook (uSenderID, sunAccount (), saAmount, sndBal);

            // Decrement SUN balance.
    ...
    {
        // Increment SUN balance.
        auto const rcvBal = receiver->getFieldAmount (sfBalance);
        receiver->setFieldAmount (sfBalance, rcvBal + saAmount);

        if (fv2Switch)
            view.creditHook (sunAccount (), uReceiverID, saAmount, -rcvBal);
    ...
    beast::Journal j)
{
    ...
    if (before > beast::zero
        ...
        // Sender reserve is set.
        && static_cast <bool> (flags & (!bSenderHigh ? lsfLowNoSUNNYCoin : lsfHighNoSUNNYCoin)) !=
           static_cast <bool> (sle->getFlags() & lsfDefaultSUNNYCoin)
...
{
    assert (!isSUN (account) && !isSUN (issue.account));
    ...
    uint256 const index = getSUNNYCoinStateIndex (
    ...
    if (!state)
    {
        ...
        bool noSUNNYCoin = (receiverAccount->getFlags() & lsfDefaultSUNNYCoin) == 0;

        return trustCreate (view, bSenderHigh, issue.account, account, index,
            receiverAccount, false, noSUNNYCoin, false, final_balance, limit, 0, 0, j);
    }
