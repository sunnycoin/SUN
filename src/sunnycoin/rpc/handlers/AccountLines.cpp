...
void addLine (Json::Value& jsonLines, SUNNYCoinState const& line)
{
    ...
    if (line.getNoSUNNYCoin ())
        jPeer[jss::no_sunnycoin] = true;
    if (line.getNoSUNNYCoinPeer ())
        jPeer[jss::no_sunnycoin_peer] = true;
    if (line.getFreeze ())
...
Json::Value doAccountLines (RPC::Context& context)
{
    ...
    if (params.isMember (jss::marker))
    {
        ...
        auto const sleLine = ledger->read({ltSUNNYCOIN_STATE, startAfter});
        ...
        auto const line = SUNNYCoinState::makeItem (accountID, sleLine);
        if (line == nullptr)
            return rpcError (rpcINVALID_PARAMS);
...

    {
        if (! forEachItemAfter(*ledger, accountID,
            ...
            {
                auto const line =
                    SUNNYCoinState::makeItem (visitData.accountID, sleCur);
                if (line != nullptr &&
                    (! visitData.hasPeer ||
    ...
    if (visitData.items.size () == reserve)
    {
        ...
        SUNNYCoinState::pointer line (visitData.items.back ());
