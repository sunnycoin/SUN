...
Json::Value doBookOffers (RPC::Context& context)
{
    ...
    {
        pay_issuer = sunAccount ();
    }

    if (isSUN (pay_currency) && ! isSUN (pay_issuer))
        return RPC::make_error (
            rpcSRC_ISR_MALFORMED, "Unneeded field 'taker_pays.issuer' for "
            "SUN currency specification.");
    ...
    {
        get_issuer = sunAccount ();
    }


    if (isSUN (get_currency) && ! isSUN (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
    ...
    if (!isSUN (get_currency) && isSUN (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
            "Invalid field 'taker_gets.issuer', expected non-SUN issuer.");
