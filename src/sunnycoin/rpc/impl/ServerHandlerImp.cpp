...
#include <sunnycoin/rpc/json_body.h>
#include <sunnycoin/rpc/ServerHandler.h>
#include <sunnycoin/server/Server.h>
#include <sunnycoin/server/impl/JSONRPCUtil.h>
#include <sunnycoin/rpc/impl/ServerHandlerImp.h>
#include <sunnycoin/basics/contract.h>
#include <sunnycoin/basics/Log.h>
#include <sunnycoin/basics/make_SSLContext.h>
...
#include <sunnycoin/server/SimpleWriter.h>
            ...
            Json::Value const& jv)
{
    ...
    try
    {
             ...
             jv[jss::command].asString() != jv[jss::method].asString()))
        {
            ...
            if (jv.isMember(jss::sunnycoinrpc))
                jr[jss::sunnycoinrpc] = jv[jss::sunnycoinrpc];
        ...
        std::string forwardedFor, std::string user)
{
    ...
    for (unsigned i = 0; i < size; ++i)
    {
        ...
        std::string sunnycoinrpc = "1.0";
        if (params.isMember(jss::sunnycoinrpc) && params[jss::sunnycoinrpc] != "1.0")
            sunnycoinrpc = params[jss::sunnycoinrpc].asString();
        /**
        ...
        if (params.isMember(jss::sunnycoinrpc))
           r[jss::sunnycoinrpc] = params[jss::sunnycoinrpc];
    ...
    http_request_type const& request) const
{
    ...
    if (app_.serverOkay(reason))
    {
        ...
        msg.body() = "<!DOCTYPE html><html><head><title>" + systemName() +
            " Test page for sunnycoind</title></head><body><h1>" +
                systemName() + " Test</h1><p>This page shows sunnycoind http(s) "
