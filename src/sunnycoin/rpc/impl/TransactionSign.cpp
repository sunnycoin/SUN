...
#include <sunnycoin/rpc/impl/TransactionSign.h>
...
#include <sunnycoin/app/tx/apply.h>              // Validity::Valid
...
#include <sunnycoin/rpc/impl/LegacyPathFind.h>
    ...
    bool doPath)
{
    ...
    if (!tx_json.isMember (jss::Paths) && params.isMember (jss::build_path))
    {
...

        {
            ...
            if (ledger)
            {
                Pathfinder pf(std::make_shared<SUNNYCoinLineCache>(ledger),
    ...
    ProcessTransactionFn const& processTransaction)
{
    ...
    // Validate the fields in the serialized transaction.
    {
        ...
        // The Fee field must be in SUN and greater than zero.
        ...
        if (!isLegalNet (fee))
        {
            ...
            err << "Invalid " << sfFee.fieldName
                << " field.  Fees must be specified in SUN.";
            return RPC::make_error (rpcINVALID_PARAMS, err.str ());
        }
        if (fee <= 0)
        {
