...
#include <sunnycoin/json/Object.h>
#include <sunnycoin/net/HTTPClient.h>
...
private:
    ...
    static Json::Value jvParseCurrencyIssuer (std::string const& strCurrencyIssuer)
    {
        ...
        if (boost::regex_match (strCurrencyIssuer, smMatch, reCurIss))
        {
            ...
            if (strIssuer.length ())
            {
                // Could confirm issuer is a valid SUNNYCoin address.
...
private:
    ...
    bool isValidJson2(Json::Value const& jv)
    {
        ...
        if (jv.isObject())
        {
            if (jv.isMember(jss::jsonrpc) && jv[jss::jsonrpc] == "2.0" &&
                jv.isMember(jss::sunnycoinrpc) && jv[jss::sunnycoinrpc] == "2.0" &&
    ...
    Json::Value parseJson2(Json::Value const& jvParams)
    {
        ...
        if (valid_parse && isValidJson2(jv))
        {
            if (jv.isObject())
            {
                ...
                jv1[jss::sunnycoinrpc] = jv[jss::sunnycoinrpc];
            ...
            for (Json::UInt j = 0; j < jv.size(); ++j)
            {
                ...
                jv1[j][jss::sunnycoinrpc] = jv[j][jss::sunnycoinrpc];
        ...
        if (jv.isMember(jss::sunnycoinrpc))
            jv_error[jss::sunnycoinrpc] = jv[jss::sunnycoinrpc];
    ...
    Json::Value parseSUNNYCoinPathFind (Json::Value const& jvParams)
    {
        Json::Reader    reader;
        Json::Value     jvRequest{Json::objectValue};
...
public:
    ...
    //      {   "profile",              &RPCParser::parseProfile,               1,  9   },
            ...
            {   "sunnycoin_path_find",     &RPCParser::parseSUNNYCoinPathFind,        1,  2   },
...
cmdLineToJSONRPC (std::vector<std::string> const& args, beast::Journal j)
{
    ...
    if (paramsObj.isMember(jss::sunnycoinrpc))
        jv[jss::sunnycoinrpc] = paramsObj[jss::sunnycoinrpc];
