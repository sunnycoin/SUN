...
{
    bool toSUN = isSUN (book.out);
    std::lock_guard <std::recursive_mutex> sl (mLock);

    if (toSUN)
    {
        // We don't want to search through all the to-SUN or from-SUN order
        // books!
        for (auto ob: mSourceMap[book.in])
        {
            if (isSUN (ob->getCurrencyOut ())) // also to SUN
                return;
        }
    }
    else
...
bool OrderBookDB::isBookToSUN(Issue const& issue)
{
    std::lock_guard <std::recursive_mutex> sl (mLock);
    return mSUNBooks.count(issue) > 0;
}
