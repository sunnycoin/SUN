...
#include <sunnycoin/app/paths/cursor/SUNNYCoinLiquidity.h>
...
// - If next node is an offer and output is non-SUN then we are the issuer and
//   do not need to push funds.
// - If next node is an offer and output is SUN then we need to deliver funds to
//   limbo.
TER PathCursor::forwardLiquidityForAccount () const
{
    TER resultCode   = tesSUCCESS;
    auto const lastNodeIndex       = pathState_.nodes().size () - 1;
    auto viewJ = sunnycoinCalc_.logs_.journal ("View");
    ...
    if (previousNode().isAccount() && nextNode().isAccount())
    {
        ...
        if (!nodeIndex_)
        {
            ...
            // For the first node, calculate amount to sunnycoin based on what is
            // available.
        ...
        else if (nodeIndex_ == lastNodeIndex)
        {
            ...
            {
                // Actually receive.
                resultCode = sunnycoinCredit(view(),
                    previousAccountID,
                    node().account_,
    ...
    else if (previousNode().isAccount() && !nextNode().isAccount())
    {
        ...
        if (nodeIndex_)
        {
            // Non-SUN, current node is the issuer.
            JLOG (j_.trace())
                << "forwardLiquidityForAccount: account --> "
                << "ACCOUNT --> offer";
        ...
        else
        {
            ...
            if (pathState_.inReq() >= beast::zero)
            {
                ...
                // Limit SUN by available. No limit for non-SUN as issuer.
                if (isSUN (node().issue_))
                    node().saFwdDeliver = std::min (
                        node().saFwdDeliver,
                        accountHolds(view(),
                            node().account_,
                            sunCurrency(),
                            sunAccount(),
                            fhIGNORE_FREEZE, viewJ)); // SUN can't be frozen
            ...
            else if (!isSUN (node().issue_))
            {
                // Non-SUN, current node is the issuer.
                // We could be delivering to multiple accounts, so we don't know
                // which sunnycoin balance will be adjusted.  Assume just issuing.
            ...
            else
            {
                ...
                // Deliver SUN to limbo.
                resultCode = accountSend(view(),
                    node().account_, sunAccount(), node().saFwdDeliver, viewJ);
            }
        }
    }
    else if (!previousNode().isAccount() && nextNode().isAccount())
