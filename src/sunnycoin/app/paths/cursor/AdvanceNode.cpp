...
TER PathCursor::advanceNode (STAmount const& amount, bool reverse, bool callerHasLiquidity) const
{
    ...
    PathCursor withMultiQuality {sunnycoinCalc_, pathState_, multi, j_, nodeIndex_};
        ...
        else
        {
            ...
            else
            {
                            ...
                            view().parentCloseTime().time_since_epoch().count()))
                {
                    ...
                    sunnycoinCalc_.permanentlyUnfundedOffers_.insert(
                        node().offerIndex_);
                    continue;
                }
                ...
                if (node().saTakerPays <= beast::zero || node().saTakerGets <= beast::zero)
                {
                    ...
                    else if (sunnycoinCalc_.permanentlyUnfundedOffers_.find (index)
                             != sunnycoinCalc_.permanentlyUnfundedOffers_.end ())
                    {
                ...
                auto itPast = sunnycoinCalc_.mumSource_.find (accountIssue);
                bool bFoundPast = (itPast != sunnycoinCalc_.mumSource_.end ());
                ...
                if (node().saOfferFunds <= beast::zero)
                {
                    ...
                    if (bReverse && !bFoundReverse && !bFoundPast)
                    {
                        ...
                        // still do deletions.
                        // Mark offer for always deletion.
                        sunnycoinCalc_.permanentlyUnfundedOffers_.insert (node().offerIndex_);
                    }
                    else
                    {
