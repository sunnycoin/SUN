...
#include <sunnycoin/app/paths/cursor/EffectiveRate.h>
    ...
    bool callerHasLiquidity) const
{
    ...
    while (resultCode == tesSUCCESS && saInAct + saInFees < saInReq)
    {
        ...
        else if (resultCode == tesSUCCESS)
        {
            ...
            if (!isSUN(nextNode().account_))
            {
                // ? --> OFFER --> account
                // Input fees: vary based upon the consumed offer's owner.
                // Output fees: none as SUN or the destination account is the
                // issuer.
                ...
                // Output: Debit offer owner, send SUN or non-XPR to next
                // account.
            ...
            else
            {
                ...
                // fees).
                auto const& id = isSUN(node().issue_) ?
                        sunAccount() : node().issue_.account;
            ...
            // is redundant.
            if (isSUN (previousNode().issue_.currency)
                || uInAccountID != node().offerOwnerAccount_)
            {
                auto id = !isSUN(previousNode().issue_.currency) ?
                        uInAccountID : sunAccount();
                resultCode = accountSend(view(),
                    id,
                    node().offerOwnerAccount_,
                    saInPassAct,
