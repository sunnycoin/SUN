...
#ifndef SUNNYCOIN_APP_PATHS_IMPL_STEP_CHECKS_H_INCLUDED
#define SUNNYCOIN_APP_PATHS_IMPL_STEP_CHECKS_H_INCLUDED
    ...
    beast::Journal j)
{
    ...
    if ((*sleIn)[sfFlags] &
        ...
        (*sleOut)[sfFlags] &
            ((cur > next) ? lsfHighNoSUNNYCoin : lsfLowNoSUNNYCoin))
    {
        JLOG (j.info()) << "Path violates noSUNNYCoin constraint between " << prev
                      << ", " << cur << " and " << next;
