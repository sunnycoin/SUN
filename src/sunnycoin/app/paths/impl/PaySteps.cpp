...
bool checkNear (SUNAmount const& expected, SUNAmount const& actual)
{
    return expected == actual;
};

static
bool isSUNAccount (STPathElement const& pe)
{
    if (pe.getNodeType () != STPathElement::typeAccount)
        return false;
    return isSUN (pe.getAccountID ());
};
    ...
    Issue const& curIssue)
{
    ...
    if (ctx.isFirst && e1->isAccount () &&
        ...
        isSUN (e1->getCurrency ()))
    {
        return make_SUNEndpointStep (ctx, e1->getAccountID ());
    }

    if (ctx.isLast && isSUNAccount (*e1) && e2->isAccount())
        return make_SUNEndpointStep (ctx, e2->getAccountID());
    ...
    if (isSUN (curIssue.currency) && isSUN (outCurrency))
    {
        JLOG (j.warn()) << "Found sun/sun offer payment step";
        return {temBAD_PATH, std::unique_ptr<Step>{}};
    }

    assert (e2->isOffer ());

    if (isSUN (outCurrency))
        return make_BookStepIX (ctx, curIssue);

    if (isSUN (curIssue.currency))
    ...
    beast::Journal j)
{
    if (isSUN (src))
    {
        JLOG (j.debug()) << "toStrand with sunAccount as src";
        return {temBAD_PATH, Strand{}};
    }
    if (isSUN (dst))
    {
        JLOG (j.debug()) << "toStrand with sunAccount as dst";
        return {temBAD_PATH, Strand{}};
    }
    ...
    beast::Journal j)
{
    ...
    for (auto const& pe : path)
    {
        ...
        if (hasIssuer && isSUN(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};

        if (hasAccount && isSUN(pe.getAccountID()))
            return {temBAD_PATH, Strand{}};

        if (hasCurrency && hasIssuer &&
            isSUN(pe.getCurrency()) != isSUN(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};
    ...
    */
        ...
        */
        ...
        if (cur->isAccount() && next->isAccount())
        {
                ...
                curIssue.account != next->getAccountID ())
            {
                ...
                result.push_back (std::move (msr.second));
                impliedPE.emplace(STPathElement::typeAccount,
                    curIssue.account, sunCurrency(), sunAccount());
                cur = &*impliedPE;
            }
        }
        else if (cur->isAccount() && next->isOffer())
        ...
        else if (cur->isOffer() && next->isAccount())
        {
                ...
                !isSUN (next->getAccountID ()))
            {
                if (isSUN(curIssue))
                {
                    ...
                    {
                        // Last step. insert sun endpoint step
                        auto msr = make_SUNEndpointStep (ctx(), next->getAccountID());
                        if (msr.first != tesSUCCESS)
                            return {msr.first, Strand{}};
                        result.push_back(std::move(msr.second));
                    }
    ...
    auto checkStrand = [&]() -> bool {
        auto stepAccts = [](Step const& s) -> std::pair<AccountID, AccountID> {
            ...
            return std::make_pair(sunAccount(), sunAccount());
        };
    ...
    beast::Journal j)
{
    ...
    {
        JLOG (j.debug())
            << "Flow: Invalid transaction: No paths and direct sunnycoin not allowed.";
        return {temSUNNYCOIN_EMPTY, std::vector<Strand>{}};
    }
...
template<>
bool
isDirectJrpToJrp<SUNAmount, SUNAmount> (Strand const& strand)
{
...
template
bool
isDirectJrpToJrp<SUNAmount, IOUAmount> (Strand const& strand);
template
bool
isDirectJrpToJrp<IOUAmount, SUNAmount> (Strand const& strand);
template
bool
