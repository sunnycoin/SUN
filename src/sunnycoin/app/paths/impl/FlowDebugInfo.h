...
#ifndef SUNNYCOIN_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
#define SUNNYCOIN_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
        ...
        if (writePassInfo)
        {
            ...
            auto writeJrpAmtList = [&write_list](
                ...
                auto get_val = [](EitherAmount const& a) -> std::string {
                    return sunnycoin::to_string (a.sun);
                };
            ...
            auto writeIouAmtList = [&write_list](
                std::vector<EitherAmount> const& amts, char delim=';') {
                auto get_val = [](EitherAmount const& a) -> std::string {
                    return sunnycoin::to_string (a.iou);
                };
                write_list (amts, get_val, delim);
            };
...
{
    return {sb.balanceChanges (rv), sb.sunDestroyed ()};
}
...
balanceDiffsToString (boost::optional<BalanceDiffs> const& bd)
{
    ...
    auto const& sunDestroyed = bd->second;
    std::ostringstream ostr;
    ostr << ", sunDestroyed: " << to_string (sunDestroyed);
    ostr << ", balanceDiffs: ";
