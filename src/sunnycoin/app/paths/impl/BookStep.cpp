...
#include <sunnycoin/app/paths/NodeDirectory.h>
...
#include <sunnycoin/protocol/Book.h>
...
public:
        ...
        bool prevStepRedeems) const
    {
        ...
        auto rate = [&](AccountID const& id) {
            if (isSUN(id) || id == this->strandDst_)
                return parityRate;
    ...
    Callback& callback) const
{
    ...
    while (offers.step ())
    {
        ...
        // An account can always own SUN or their own IOUs.
        if (flowCross &&
            (!isSUN (offer.issueIn().currency)) &&
        ...
        if (ctx.prevStep)
        {
            if (auto const prev = ctx.prevStep->directStepSrcAcct())
            {
                ...
                if (!sle)
                    return terNO_LINE;
                if ((*sle)[sfFlags] &
                    ((cur > *prev) ? lsfHighNoSUNNYCoin : lsfLowNoSUNNYCoin))
                    return terNO_SUNNYCOIN;
            }
        }
    }
...
template <class TIn, class TOut, class TDerived>
static
bool equalHelper (Step const& step, sunnycoin::Book const& book)
{
...
bool bookStepEqual (Step const& step, sunnycoin::Book const& book)
{
    bool const inSUN = isSUN (book.in.currency);
    bool const outSUN = isSUN (book.out.currency);
    if (inSUN && outSUN)
        return equalHelper<SUNAmount, SUNAmount,
            BookPaymentStep<SUNAmount, SUNAmount>> (step, book);
    if (inSUN && !outSUN)
        return equalHelper<SUNAmount, IOUAmount,
            BookPaymentStep<SUNAmount, IOUAmount>> (step, book);
    if (!inSUN && outSUN)
        return equalHelper<IOUAmount, SUNAmount,
            BookPaymentStep<IOUAmount, SUNAmount>> (step, book);
    if (!inSUN && !outSUN)
...
make_BookStepIX (
    ...
    Issue const& in)
{
    return make_BookStepHelper<IOUAmount, SUNAmount> (ctx, in, sunIssue());
}
...
make_BookStepXI (
    ...
    Issue const& out)
{
    return make_BookStepHelper<SUNAmount, IOUAmount> (ctx, sunIssue(), out);
}
