...
#include <sunnycoin/app/paths/PathState.h>
...
class SUNNYCoinCalc; // for logging
...
// - A sunnycoin nodes output issuer must be the node's account or the next node's
//   account.
// - Offers can only go directly to another offer if the currency and issuer are
//   an exact match.
// - Real issuers must be specified for non-SUN.
    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    ...
    if (nodes_.back ().issue_.currency != currency)
    {
        ...
        // book.  sunAccount() does double duty as signaling "this is an order
        // book".

        // Corresponds to "Implies an offer directory" in the diagram, currently
        // at http://goo.gl/Uj3HAB.

        auto type = isSUN(currency) ? STPathElement::typeCurrency
            : STPathElement::typeCurrency | STPathElement::typeIssuer;

        // The offer's output is what is now wanted.
        // sunAccount() is a placeholder for offers.
        resultCode = pushNode (type, sunAccount(), currency, issuer);
    }


    // For sunnycoin, non-SUN, ensure the issuer is on at least one side of the
    // transaction.
    ...
    {
        // Need to sunnycoin through issuer's account.
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    // true, iff node is a sunnycoin account. false, iff node is an offer node.
    ...
    else if (hasIssuer && isSUN (node.issue_))
    {
        JLOG (j_.debug()) << "pushNode: issuer specified for SUN.";
    ...
    else if (hasAccount)
    {
        ...
        node.issue_.account = hasIssuer ? issuer :
                (isSUN (node.issue_) ? sunAccount() : account);
        ...
        else
        {
            ...
            resultCode = pushImpliedNodes (
                ...
                isSUN(node.issue_.currency) ? sunAccount() : account);
        ...
        if (resultCode == tesSUCCESS && !nodes_.empty ())
        {
            ...
            {
                auto sleSUNNYCoinState = view().peek(
                    keylet::line(backNode.account_, node.account_, backNode.issue_.currency));

                // A "SUNNYCoinState" means a balance betweeen two accounts for a
                // specific currency.
                ...
                else
                {
                    ...
                    if (!sleBck)
                    {
                        ...
                        resultCode   = terNO_ACCOUNT;
                             ...
                             !(sleSUNNYCoinState->getFieldU32 (sfFlags) &
                                  (bHigh ? lsfHighAuth : lsfLowAuth)) &&
                             sleSUNNYCoinState->getFieldAmount(sfBalance) == beast::zero)
                    {
                        JLOG (j_.warn())
                                << "pushNode: delay: can't receive IOUs from "
                                << "issuer without auth.";
    ...
    else
    {
        ...
        else if (isSUN (node.issue_.currency))
            node.issue_.account = sunAccount();
        else if (isSUN (backNode.issue_.account))
        ...
        else if (backNode.issue_ == node.issue_)
        {
            ...
            resultCode   = pushImpliedNodes (
                sunAccount(), // Rippling, but offers don't have an account.
    ...
    AccountID const& uSenderID)
{
    ...
    AccountID const& uSenderIssuerID
        = isSUN(uMaxCurrencyID) ? sunAccount() : uSenderID;
    // Sender is always issuer for non-SUN.
    ...
    // SUN with issuer is malformed.
    if ((isSUN (uMaxCurrencyID) && !isSUN (uMaxIssuerID))
        || (isSUN (currencyOutID) && !isSUN (issuerOutID)))
    {
        JLOG (j_.debug())
            << "expandPath> issuer with SUN";
        terStatus   = temBAD_PATH;
    }

    // Push sending node.
    // For non-SUN, issuer is always sending account.
    // - Trying to expand, not-compact.
    // - Every issuer will be traversed through.
    if (terStatus == tesSUCCESS)
    {
        terStatus   = pushNode (
            !isSUN(uMaxCurrencyID)
    ...
    if (tesSUCCESS == terStatus && uMaxIssuerID != uSenderIssuerID)
    {
        ...
        // - If it was SUN, then issuers would have matched.
        ...
        // account.
        if (!uNxtCurrencyID
            // Next is SUN, offer next. Must go through issuer.
    ...
    if (terStatus == tesSUCCESS
        && !isSUN(currencyOutID)               // Next is not SUN
...
/** Check if a sequence of three accounts violates the no sunnycoin constrains
    [first] -> [second] -> [third]
    Disallowed if 'second' set no sunnycoin on [first]->[second] and
    [second]->[third]
*/
TER PathState::checkNoSUNNYCoin (
...
{
    // fetch the sunnycoin lines into and out of this node
    ...
    if (!sleIn || !sleOut)
    {
        ...
        sleIn->getFieldU32 (sfFlags) &
            ((secondAccount > firstAccount) ? lsfHighNoSUNNYCoin : lsfLowNoSUNNYCoin) &&
        sleOut->getFieldU32 (sfFlags) &
            ((secondAccount > thirdAccount) ? lsfHighNoSUNNYCoin : lsfLowNoSUNNYCoin))
    {
        JLOG (j_.info())
            << "Path violates noSUNNYCoin constraint between "
            << firstAccount << ", "
            << secondAccount << " and "
            << thirdAccount;

        terStatus = terNO_SUNNYCOIN;
    }
    return terStatus;
}

// Check a fully-expanded path to make sure it doesn't violate no-SUNNYCoin
// settings.
TER PathState::checkNoSUNNYCoin (
    AccountID const& uDstAccountID,
    AccountID const& uSrcAccountID)
{
    // There must be at least one node for there to be two consecutive sunnycoin
    // lines.
    if (nodes_.size() == 0)
       return terStatus;
    ...
    // These are the nodes whose no sunnycoin constraints could be violated
