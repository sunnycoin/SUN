...
#include <sunnycoin/app/paths/Node.h>
...
Json::Value Node::getJson () const
{
    ...
    bool const hasCurrency = !isSUN (issue_.currency);
    bool const hasAccount = !isSUN (account_);
    bool const hasIssuer = !isSUN (issue_.account);
    ...
    if (!isSUN (issue_.currency))
        jvNode[jss::currency] = to_string (issue_.currency);

    if (!isSUN (issue_.account))
