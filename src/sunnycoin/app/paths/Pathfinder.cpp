...
#include <sunnycoin/app/paths/Tuning.h>
#include <sunnycoin/app/paths/Pathfinder.h>
...
#include <sunnycoin/app/ledger/OrderBookDB.h>
#include <sunnycoin/basics/Log.h>
#include <sunnycoin/json/to_string.h>
...
The pathfinding request is identified by category, SUN to SUN, SUN to
non-SUN, non-SUN to SUN, same currency non-SUN to non-SUN, cross-currency
non-SUN to non-SUN.  For each category, there is a table of paths that the
...
Pathfinder::Pathfinder (
    ...
    :   mSrcAccount (uSrcAccount),
        ...
        mEffectiveDst (isSUN(saDstAmount.getIssuer ()) ?
        ...
        mSrcAmount (srcAmount.value_or(STAmount({uSrcCurrency,
            uSrcIssuer.value_or(isSUN(uSrcCurrency) ?
                sunAccount() : uSrcAccount)}, 1u, 0, true))),
...
{
    assert (! uSrcIssuer || isSUN(uSrcCurrency) == isSUN(uSrcIssuer.get()));
}
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    auto currencyIsSUN = isSUN (mSrcCurrency);

    bool useIssuerAccount
            = mSrcIssuer && !currencyIsSUN && !isSUN (*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsSUN ? AccountID() : account;
    ...
    bool bSrcJrp = isSUN (mSrcCurrency);
    bool bDstJrp = isSUN (mDstAmount.getCurrency());
    ...
    if (! mLedger->exists (keylet::account (mDstAccount)))
    {
        ...
        {
            JLOG (j_.debug())
                    << "New account not being funded in SUN ";
            return false;
        }
    ...
    {
        // SUN -> SUN
        JLOG (j_.debug()) << "SUN to SUN payment";
        paymentType = pt_SUN_to_SUN;
    }
    else if (bSrcJrp)
    {
        // SUN -> non-SUN
        JLOG (j_.debug()) << "SUN to non-SUN payment";
        paymentType = pt_SUN_to_nonSUN;
    }
    else if (bDstJrp)
    {
        // non-SUN -> SUN
        JLOG (j_.debug()) << "non-SUN to SUN payment";
        paymentType = pt_nonSUN_to_SUN;
    }
    else if (mSrcCurrency == mDstAmount.getCurrency ())
    {
        // non-SUN -> non-SUN - Same currency
        JLOG (j_.debug()) << "non-SUN to non-SUN - same currency";
        paymentType = pt_nonSUN_to_same;
    }
    else
    {
        // non-SUN to non-SUN - Different currency
        JLOG (j_.debug()) << "non-SUN to non-SUN - cross currency";
        paymentType = pt_nonSUN_to_nonSUN;
    }
    ...
    uint64_t& qualityOut) const    // OUT: The returned initial quality
{
    ...
    path::SUNNYCoinCalc::Input rcInput;
    ...
    try
    {
        ...
        auto rc = path::SUNNYCoinCalc::sunnycoinCalculate (
            sandbox,
            mSrcAmount,
            minDstAmount,
            mDstAccount,
        ...
        if (! convert_all_)
        {
            ...
            rcInput.partialPaymentAllowed = true;
            rc = path::SUNNYCoinCalc::sunnycoinCalculate (
                sandbox,
                mSrcAmount,
...
static bool isDefaultPath (STPath const& path)
{
    ...
    // https://forum.sunnycoin.com/viewtopic.php?f=2&t=8206&start=10#p57713
    //
    // JoelKatz writes:
    ...
    AccountID const& srcIssuer)
{
    ...
    const bool issuerIsSender = isSUN (mSrcCurrency) || (srcIssuer == mSrcAccount);
    ...
    AccountID const& dstAccount)
{
    ...
    if (!bFrozen)
    {
        ...
        for (auto const& item : mRLCache->getSUNNYCoinLines (account))
        {
            SUNNYCoinState* rspEntry = (SUNNYCoinState*) item.get ();
            ...
            else if (rspEntry->getNoSUNNYCoinPeer ())
            {
...
STPathSet& Pathfinder::addPathsForType (PathType const& pathType)
{
    ...
    case nt_SUN_BOOK:
        addLinks (parentPaths, pathsOut, afADD_BOOKS | afOB_SUN);
        break;
...
bool Pathfinder::isNoSUNNYCoin (
    AccountID const& fromAccount,
    AccountID const& toAccount,
    Currency const& currency)
{
    auto sleSUNNYCoin = mLedger->read(keylet::line(
        toAccount, fromAccount, currency));

    auto const flag ((toAccount > fromAccount)
                     ? lsfHighNoSUNNYCoin : lsfLowNoSUNNYCoin);

    return sleSUNNYCoin && (sleSUNNYCoin->getFieldU32 (sfFlags) & flag);
}

// Does this path end on an account-to-account link whose last account has
// set "no sunnycoin" on the link?
bool Pathfinder::isNoSUNNYCoinOut (STPath const& currentPath)
{
    ...
    // no sunnycoin on the output. A path with no sunnycoin on its output can't be
    // followed by a link with no sunnycoin on its input.
    auto const& fromAccount = (currentPath.size () == 1)
        ? mSrcAccount
        : (currentPath.end () - 2)->getAccountID ();
    auto const& toAccount = endElement.getAccountID ();
    return isNoSUNNYCoin (fromAccount, toAccount, endElement.getCurrency ());
}
    ...
    int addFlags)
{
    ...
    bool const bOnSUN = uEndCurrency.isZero ();
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        if (bOnSUN)
        {
            ...
            { // non-default path to SUN destination
                JLOG (j_.trace())
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                bool const bIsNoSUNNYCoinOut (
                    isNoSUNNYCoinOut (currentPath));
                bool const bDestOnly (
                    addFlags & afAC_LAST);

                auto& sunnycoinLines (mRLCache->getSUNNYCoinLines (uEndAccount));

                AccountCandidates candidates;
                candidates.reserve (sunnycoinLines.size ());

                for (auto const& item : sunnycoinLines)
                {
                    auto* rs = dynamic_cast<SUNNYCoinState const *> (item.get ());
                    if (!rs)
                    {
                        JLOG (j_.error())
                                << "Couldn't decipher SUNNYCoinState";
                        continue;
                    }
                        ...
                        !currentPath.hasSeen (acct, uEndCurrency, acct))
                    {
                        ...
                        else if (bIsNoSUNNYCoinOut && rs->getNoSUNNYCoin ())
                        {
    ...
    {
        // add order books
        if (addFlags & afOB_SUN)
        {
            // to SUN only
            if (!bOnSUN && app_.getOrderBookDB ().isBookToSUN (
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut () == mDstAmount.getCurrency ())))
                {
                    ...
                    { // to SUN
                        ...
                        {
                            // destination is SUN, add account and path is
                            // complete
                            JLOG (j_.trace())
                        ...
                        book->getIssuerOut ()))
                    {
                            ...
                            (newPath[newPath.size() - 2].isOffer ()))
                        {
                            ...
                            newPath[newPath.size() - 1] = STPathElement (
                                ...
                                sunAccount(), book->getCurrencyOut(),
                                book->getIssuerOut());
                        }
                        else
                        {
...
Pathfinder::PathType makePath (char const *string)
{
    ...
    while (true)
    {
        switch (*string++)
        {
            ...
            case 'x': // sun book
                ret.push_back (Pathfinder::nt_SUN_BOOK);
                break;
...
void Pathfinder::initPathTable ()
{
    ...
    fillPaths(
        pt_nonSUN_to_SUN, {
            {1, "sxd"},       // gateway buys SUN
            {2, "saxd"},      // source -> gateway -> book(SUN) -> dest
            {6, "saaxd"},
            {7, "sbxd"},
            {8, "sabxd"},
            {9, "sabaxd"}
    ...
    // non-SUN to non-SUN (same currency)
    fillPaths(
        pt_nonSUN_to_same,  {
            ...
            {6, "sxfad"},
            {6, "safad"},
            {6, "saxfd"},   // source -> gateway -> book to SUN -> book ->
                            // destination
            {6, "saxfad"},
