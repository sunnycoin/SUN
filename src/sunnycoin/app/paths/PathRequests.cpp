...
/** Get the current SUNNYCoinLineCache, updating it if necessary.
    Get the correct ledger to use.
*/
std::shared_ptr<SUNNYCoinLineCache>
    ...
    bool authoritative)
{
    ...
    {
        mLineCache = std::make_shared<SUNNYCoinLineCache> (ledger);
    }
    return mLineCache;
}
                              ...
                              Job::CancelCallback shouldCancel)
{
    ...
    std::shared_ptr<SUNNYCoinLineCache> cache;
...
// Make an old-style sunnycoin_path_find request
Json::Value
...
PathRequests::doLegacyPathRequest (
        ...
        Json::Value const& request)
{
    auto cache = std::make_shared<SUNNYCoinLineCache> (inLedger);
