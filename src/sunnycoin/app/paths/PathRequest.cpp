...
#include <sunnycoin/protocol/UintTypes.h>
...
bool PathRequest::isValid (std::shared_ptr<SUNNYCoinLineCache> const& crCache)
{
    ...
    if (! sleDest)
    {
        ...
        {
            // Only SUN can be send to a non-existent account.
    ...
    {
        bool const disallowSUN (
            sleDest->getFlags() & lsfDisallowSUN);

        auto usDestCurrID = accountDestCurrencies (
                *raDstAccount, crCache, ! disallowSUN);
...
PathRequest::getPathFinder(std::shared_ptr<SUNNYCoinLineCache> const& cache,
...
bool
PathRequest::findPaths (std::shared_ptr<SUNNYCoinLineCache> const& cache,
    int const level, Json::Value& jvArray)
{
        ...
        for (auto const& c : currencies)
        {
            if (! sameAccount || c != saDstAmount.getCurrency())
            {
                ...
                sourceCurrencies.insert(
                    {c, c.isZero() ? sunAccount() : *raSrcAccount});
            }
        }
    }
    ...
    for (auto const& issue : sourceCurrencies)
    {
        ...
        auto& sourceAccount = ! isSUN(issue.account)
            ? issue.account
        ...
        JLOG(m_journal.debug()) << iIdentifier
            << " Paths found, calling sunnycoinCalc";
        ...
        if (rc.result () == tesSUCCESS)
        {
            ...
            if (hasCompletion ())
            {
                // Old sunnycoin_path_find API requires this
        ...
        {
            JLOG(m_journal.debug()) << iIdentifier << " sunnycoinCalc returns "
                << transHuman(rc.result());
        }
    }
...
Json::Value PathRequest::doUpdate(
    std::shared_ptr<SUNNYCoinLineCache> const& cache, bool fast)
{
    using namespace std::chrono;
    JLOG(m_journal.debug()) << iIdentifier
    ...
    if (hasCompletion ())
    {
        // Old sunnycoin_path_find API gives destination_currencies
