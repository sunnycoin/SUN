...
#include <sunnycoin/app/paths/cursor/PathCursor.h>
#include <sunnycoin/app/paths/impl/FlowDebugInfo.h>
...
SUNNYCoinCalc::Output SUNNYCoinCalc::sunnycoinCalculate (
    PaymentSandbox& view,
    ...
    // Issuer:
    //      SUN: sunAccount()
    //  non-SUN: uSrcAccountID (for any issuer) or another account with
    //           trust node.
    STAmount const& saMaxAmountReq,             // --> -1 = no limit.

    // Issuer:
    //      SUN: sunAccount()
    //  non-SUN: uDstAccountID (for any issuer) or another account with
    //           trust node.
    ...
    Input const* const pInputs)
{
    ...
    if (callFlowV1)
    {
        ...
        auto result = rc.sunnycoinCalculate (compareFlowV1V2 ? &flowV1FlowDebugInfo : nullptr);
    ...
    if (callFlowV2)
    {
        ...
        catch (std::exception& e)
        {
            ...
            if (!useFlowV1Output)
            {
                ...
                path::SUNNYCoinCalc::Output exceptResult;
...
bool SUNNYCoinCalc::addPathState(STPath const& path, TER& resultCode)
{
    ...
    if (pathState->status() == tesSUCCESS)
        pathState->checkNoSUNNYCoin (uDstAccountID_, uSrcAccountID_);
...
TER SUNNYCoinCalc::sunnycoinCalculate (detail::FlowDebugInfo* flowDebugInfo)
{
    JLOG (j_.trace())
        << "sunnycoinCalc>"
    ...
    {
        JLOG (j_.debug())
            << "sunnycoinCalc: Invalid transaction:"
            << "No paths and direct sunnycoin not allowed.";

        return temSUNNYCOIN_EMPTY;
    }

    // Build a default path.  Use saDstAmountReq_ and saMaxAmountReq_ to imply
    // nodes.
    // XXX Might also make a SUN bridge by default.

    JLOG (j_.trace())
        << "sunnycoinCalc: Paths in set: " << spsPaths_.size ();
        ...
        for (auto pathState : pathStateList_)
        {
                ...
                // Only do active paths.
            {
                ...
                else if (pathState->outPass() == beast::zero)
                {
                    ...
                    JLOG (j_.warn())
                        << "sunnycoinCalc: Non-dry path moves no funds";
