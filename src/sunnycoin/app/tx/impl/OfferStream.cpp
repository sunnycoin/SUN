...
static
SUNAmount accountFundsHelper (ReadView const& view,
    AccountID const& id,
    SUNAmount const& amtDefault,
    Issue const& issue,
    FreezeHandling freezeHandling,
    beast::Journal j)
{
    return toAmount<SUNAmount> (
        accountHolds (view, id, issue.currency, issue.account, freezeHandling, j));
}
...
template class FlowOfferStream<SUNAmount, IOUAmount>;
template class FlowOfferStream<IOUAmount, SUNAmount>;

template class TOfferStreamBase<STAmount, STAmount>;
template class TOfferStreamBase<IOUAmount, IOUAmount>;
template class TOfferStreamBase<SUNAmount, IOUAmount>;
template class TOfferStreamBase<IOUAmount, SUNAmount>;
}
