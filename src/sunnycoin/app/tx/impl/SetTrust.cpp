...
#include <sunnycoin/app/tx/impl/SetTrust.h>
...
SetTrust::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.trace()) <<
            "Malformed transaction: specifies SUN as IOU";
...
SetTrust::doApply ()
{
    ...
    // which needs trust lines, is a powerful SUNNYCoin feature.
    ...
    // user enough SUN to cover not only the account reserve
    // but the incremental reserve for the trust line as
    // well. A person with no intention of using the gateway
    // could use the extra SUN for their own purposes.

    SUNAmount const reserveCreate ((uOwnerCount < 2)
    ...
    bool const bSetAuth = (uTxFlags & tfSetfAuth);
    bool const bSetNoSUNNYCoin = (uTxFlags & tfSetNoSUNNYCoin);
    bool const bClearNoSUNNYCoin  = (uTxFlags & tfClearNoSUNNYCoin);
    ...
    SLE::pointer sleSUNNYCoinState = view().peek (
        keylet::line(account_, uDstAccountID, currency));
    ...
    if (sleSUNNYCoinState)
    {
        ...
        saLowBalance    = sleSUNNYCoinState->getFieldAmount (sfBalance);
        ...
        sleSUNNYCoinState->setFieldAmount (!bHigh ? sfLowLimit : sfHighLimit, saLimitAllow);

        saLowLimit  = !bHigh ? saLimitAllow : sleSUNNYCoinState->getFieldAmount (sfLowLimit);
        saHighLimit =  bHigh ? saLimitAllow : sleSUNNYCoinState->getFieldAmount (sfHighLimit);
        ...
        if (!bQualityIn)
        {
            ...
            uLowQualityIn   = sleSUNNYCoinState->getFieldU32 (sfLowQualityIn);
            uHighQualityIn  = sleSUNNYCoinState->getFieldU32 (sfHighQualityIn);
        }
        else if (uQualityIn)
        {
            // Setting.

            sleSUNNYCoinState->setFieldU32 (!bHigh ? sfLowQualityIn : sfHighQualityIn, uQualityIn);

            uLowQualityIn   = !bHigh ? uQualityIn : sleSUNNYCoinState->getFieldU32 (sfLowQualityIn);
            uHighQualityIn  =  bHigh ? uQualityIn : sleSUNNYCoinState->getFieldU32 (sfHighQualityIn);
        }
        else
        {
            // Clearing.

            sleSUNNYCoinState->makeFieldAbsent (!bHigh ? sfLowQualityIn : sfHighQualityIn);

            uLowQualityIn   = !bHigh ? 0 : sleSUNNYCoinState->getFieldU32 (sfLowQualityIn);
            uHighQualityIn  =  bHigh ? 0 : sleSUNNYCoinState->getFieldU32 (sfHighQualityIn);
        }
        ...
        if (!bQualityOut)
        {
            ...
            uLowQualityOut  = sleSUNNYCoinState->getFieldU32 (sfLowQualityOut);
            uHighQualityOut = sleSUNNYCoinState->getFieldU32 (sfHighQualityOut);
        }
        else if (uQualityOut)
        {
            // Setting.

            sleSUNNYCoinState->setFieldU32 (!bHigh ? sfLowQualityOut : sfHighQualityOut, uQualityOut);

            uLowQualityOut  = !bHigh ? uQualityOut : sleSUNNYCoinState->getFieldU32 (sfLowQualityOut);
            uHighQualityOut =  bHigh ? uQualityOut : sleSUNNYCoinState->getFieldU32 (sfHighQualityOut);
        }
        else
        {
            // Clearing.

            sleSUNNYCoinState->makeFieldAbsent (!bHigh ? sfLowQualityOut : sfHighQualityOut);

            uLowQualityOut  = !bHigh ? 0 : sleSUNNYCoinState->getFieldU32 (sfLowQualityOut);
            uHighQualityOut =  bHigh ? 0 : sleSUNNYCoinState->getFieldU32 (sfHighQualityOut);
        }

        std::uint32_t const uFlagsIn (sleSUNNYCoinState->getFieldU32 (sfFlags));
        std::uint32_t uFlagsOut (uFlagsIn);

        if (bSetNoSUNNYCoin && !bClearNoSUNNYCoin && (bHigh ? saHighBalance : saLowBalance) >= beast::zero)
        {
            uFlagsOut |= (bHigh ? lsfHighNoSUNNYCoin : lsfLowNoSUNNYCoin);
        }
        else if (bClearNoSUNNYCoin && !bSetNoSUNNYCoin)
        {
            uFlagsOut &= ~(bHigh ? lsfHighNoSUNNYCoin : lsfLowNoSUNNYCoin);
        }
        ...
        bool const bLowDefSUNNYCoin        = sleLowAccount->getFlags() & lsfDefaultSUNNYCoin;
        bool const bHighDefSUNNYCoin       = sleHighAccount->getFlags() & lsfDefaultSUNNYCoin;

        bool const  bLowReserveSet      = uLowQualityIn || uLowQualityOut ||
                                            ((uFlagsOut & lsfLowNoSUNNYCoin) == 0) != bLowDefSUNNYCoin ||
                                            (uFlagsOut & lsfLowFreeze) ||
                                            saLowLimit || saLowBalance > beast::zero;
        bool const  bLowReserveClear    = !bLowReserveSet;

        bool const  bHighReserveSet     = uHighQualityIn || uHighQualityOut ||
                                            ((uFlagsOut & lsfHighNoSUNNYCoin) == 0) != bHighDefSUNNYCoin ||
        ...
        if (uFlagsIn != uFlagsOut)
            sleSUNNYCoinState->setFieldU32 (sfFlags, uFlagsOut);
        ...
        if (bDefault || badCurrency() == currency)
        {
            ...
            terResult = trustDelete (view(),
                sleSUNNYCoinState, uLowAccountID, uHighAccountID, viewJ);
        }
        ...
        else if (bReserveIncrease && mPriorBalance < reserveCreate)
        {
            ...
            // Another transaction could provide SUN to the account and then
        ...
        else
        {
            ...
            JLOG(j_.trace()) << "Modify sunnycoin line";
        }
    }
    // Line does not exist.
    ...
    {
        JLOG(j_.trace()) <<
            "Redundant: Setting non-existent sunnycoin line to defaults.";
    ...
    else
    {
        ...
        uint256 index (getSUNNYCoinStateIndex (
            account_, uDstAccountID, currency));

        JLOG(j_.trace()) <<
            "doTrustSet: Creating sunnycoin line: " <<
            to_string (index);

        // Create a new sunnycoin line.
        terResult = trustCreate (view(),
            ...
            sle,
            bSetAuth,
            bSetNoSUNNYCoin && !bClearNoSUNNYCoin,
