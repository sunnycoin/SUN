...
#include <sunnycoin/app/tx/impl/SignerEntries.h>
...
preflight1 (PreflightContext const& ctx)
{
    ...
    if (!fee.native () || fee.negative () || !isLegalAmount (fee.sun ()))
    {
...
TER Transactor::payFee ()
{
    ...
    // VFALCO Should we call view().rawDestroySUN() here as well?
...
TER Transactor::apply ()
{
    ...
    if (sle)
    {
        mPriorBalance   = STAmount ((*sle)[sfBalance]).sun ();
...
SUNAmount
Transactor::reset(SUNAmount fee)
{
    ctx_.discard();
    ...
    auto const balance = txnAcct->getFieldAmount (sfBalance).sun ();
...
Transactor::operator()()
{
    ...
    auto fee = ctx_.tx.getFieldAmount(sfFee).sun ();
