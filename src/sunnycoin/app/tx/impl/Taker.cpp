    ...
    , journal_ (journal)
{
    ...
    assert (cross_type != CrossType::JrpToIou ||
        (isSUN (issue_in ()) && !isSUN (issue_out ())));

    assert (cross_type != CrossType::IouToJrp ||
        (!isSUN (issue_in ()) && isSUN (issue_out ())));

    // And make sure we're not crossing SUN for SUN
    assert (!isSUN (issue_in ()) || !isSUN (issue_out ()));
...
BasicTaker::do_cross (Amounts offer, Quality quality, AccountID const& owner)
{
    ...
    {
        result = flow_sun_to_iou (offer, quality, owner_funds, taker_funds,
            out_rate (owner, account ()));
    }
    else if (cross_type_ == CrossType::IouToJrp)
    {
        result = flow_iou_to_sun (offer, quality, owner_funds, taker_funds,
    ...
    Amounts offer2, Quality quality2, AccountID const& owner2)
{
    ...
    // The amount available to flow via SUN is the amount that the owner of the
    ...
    // of SUN that can flow between the two legs is, essentially, infinite
    // since all the owner is doing is taking out SUN of his left pocket
    // and putting it in his right pocket. In that case, we set the available
    // SUN to the largest of the two offers.
    auto sun_funds = get_funds (owner1, offer1.out);

    if (owner1 == owner2)
    {
        JLOG(journal_.trace()) <<
            "The bridge endpoints are owned by the same account.";
        sun_funds = std::max (offer1.out, offer2.in);
    }
    ...
    if (auto stream = journal_.debug())
    {
        ...
        stream << "      sun: " << format_amount (sun_funds);
    }
    ...
    auto flow1 = flow_iou_to_sun (offer1, quality1, sun_funds, leg1_in_funds, leg1_rate);

    if (!flow1.sanity_check ())
        Throw<std::logic_error> ("Computed flow1 fails sanity check.");

    auto flow2 = flow_sun_to_iou (offer2, quality2, leg2_out_funds, sun_funds, leg2_rate);
    ...
    // equalize them, so that the amount of SUN that flows out of the first leg
    // is the same as the amount of SUN that flows into the second leg. We take
...
TER Taker::transferSUN (
    AccountID const& from,
    AccountID const& to,
    STAmount const& amount)
{
    if (!isSUN (amount))
        Throw<std::logic_error> ("Using transferSUN with IOU");
    ...
    return sunnycoin::transferSUN (view_, from, to, amount, journal_);
}
...
{
    if (isSUN (amount))
        Throw<std::logic_error> ("Using redeemIOU with SUN");
    ...
    auto ret = sunnycoin::redeemIOU (view_, account, amount, issue, journal_);
...
{
    if (isSUN (amount))
        Throw<std::logic_error> ("Using issueIOU with SUN");
    ...
    return sunnycoin::issueIOU (view_, account, amount, issue, journal_);
}
...
Taker::fill (BasicTaker::Flow const& flow, Offer& offer)
{
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferSUN (offer.owner (), account (), flow.order.out);
    }
    ...
    BasicTaker::Flow const& flow2, Offer& leg2)
{
    ...
    // leg1 to leg2: bridging over SUN
    if (result == tesSUCCESS)
        result = transferSUN (leg1.owner (), leg2.owner (), flow1.order.out);
...
{
    // In direct crossings, at least one leg must not be SUN.
    if (isSUN (offer.amount ().in) && isSUN (offer.amount ().out))
        return tefINTERNAL;
...
{
    // In bridged crossings, SUN must can't be the input to the first leg
    // or the output of the second leg.
    if (isSUN (leg1.amount ().in) || isSUN (leg2.amount ().out))
        return tefINTERNAL;
...
Taker::calculateRate (
    ApplyView const& view,
        AccountID const& issuer,
            AccountID const& account)
{
    return isSUN (issuer) || (account == issuer)
        ? parityRate
