...
#include <sunnycoin/app/tx/impl/Payment.h>
...
// See https://sunnycoin.com/wiki/Transaction_Format#Payment_.280.29
...
Payment::calculateMaxSpend(STTx const& tx)
{
    if (tx.isFieldPresent(sfSendMax))
    {
        ...
        return sendMax.native() ? sendMax.sun() : beast::zero;
    }
    /* If there's no sfSendMax in SUN, and the sfAmount isn't
    in SUN, then the transaction can not send SUN. */
    auto const& saDstAmount = tx.getFieldAmount(sfAmount);
    return saDstAmount.native() ? saDstAmount.sun() : beast::zero;
}
...
Payment::preflight (PreflightContext const& ctx)
{
    ...
    bool const defaultPathsAllowed = !(uTxFlags & tfNoSUNNYCoinDirect);
    ...
    // isZero() is SUN.  FIX!
    bool const bSUNDirect = uSrcCurrency.isZero () && uDstCurrency.isZero ();
    ...
    if (bSUNDirect && bMax)
    {
        ...
        JLOG(j.trace()) << "Malformed transaction: " <<
            "SendMax specified for SUN to SUN.";
        return temBAD_SEND_SUN_MAX;
    }
    if (bSUNDirect && bPaths)
    {
        // SUN is sent without paths.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Paths specified for SUN to SUN.";
        return temBAD_SEND_SUN_PATHS;
    }
    if (bSUNDirect && partialPaymentAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Partial payment specified for SUN to SUN.";
        return temBAD_SEND_SUN_PARTIAL;
    }
    if (bSUNDirect && limitQuality)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Limit quality specified for SUN to SUN.";
        return temBAD_SEND_SUN_LIMIT;
    }
    if (bSUNDirect && !defaultPathsAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "No sunnycoin direct specified for SUN to SUN.";
        return temBAD_SEND_SUN_NO_DIRECT;
    }
...
{
    // SUNNYCoin if source or destination is non-native or if there are paths.
    ...
    {
        // SUNNYCoin payment with at least one intermediate step and uses
        // transitive balances.
...
Payment::doApply ()
{
    ...
    bool const bSUNNYCoin = paths || sendMax || !saDstAmount.native ();

    // If the destination has lsfDepositAuth set, then only direct SUN
    // payments (no intermediate steps) are allowed to the destination.
    if (!depositPreauth && bSUNNYCoin && reqDepositAuth)
    ...
    if (bSUNNYCoin)
    {
        ...
        path::SUNNYCoinCalc::Output rc;
        {
            PaymentSandbox pv(&view());
            JLOG(j_.debug())
                << "Entering SUNNYCoinCalc in payment: "
        ...
        // Because of its overhead, if SUNNYCoinCalc
    ...
    auto const mmm = std::max(reserve,
        ctx_.tx.getFieldAmount (sfFee).sun ());

    if (mPriorBalance < saDstAmount.sun () + mmm)
    {
        // Vote no. However the transaction might succeed, if applied in
        // a different order.
        JLOG(j_.trace()) << "Delay transaction: Insufficient funds: " <<
            " " << to_string (mPriorBalance) <<
            " / " << to_string (saDstAmount.sun () + mmm) <<
            " (" << to_string (reserve) << ")";
    ...
    {
        // If depositPreauth is enabled, then an account that requires
        // authorization has three ways to get an SUN Payment in:
        //  1. If Account == Destination, or
        //  2. If Account is deposit preauthorized by destination, or
        //  3. If the destination's SUN balance is
        ...
        // then consumes all of its SUN.  Without the rule if an
        // account with lsfDepositAuth set spent all of its SUN, it
        // would be unable to acquire more SUN required to pay fees.
        //
        ...
        if (uDstAccountID != account_)
        {
                ...
                keylet::depositPreauth (uDstAccountID, account_)))
            {
                ...
                SUNAmount const dstReserve {view().fees().accountReserve (0)};
