...
#include <sunnycoin/protocol/STAccount.h>
...
CashCheck::preclaim (PreclaimContext const& ctx)
{
    ...
    }
    {
        ...
        // less than value the check cannot be cashed.
        {
            ...
            // Note that src will have one reserve's worth of additional SUN
            // once the check is cashed, since the check's reserve will no
            // longer be required.  So, if we're dealing in SUN, we add one
            // reserve's worth to the available funds.
            if (value.native())
                availableFunds += SUNAmount (ctx.view.fees().increment);
...
CashCheck::doApply ()
{
    ...
    if (srcId != account_)
    {
        ...
        // Flow() doesn't do SUN to SUN transfers.
        if (sendMax.native())
        {
            // Here we need to calculate the amount of SUN sleSrc can send.
            ...
            // argument.
            STAmount const srcLiquid {sunLiquid (psb, srcId, -1, viewJ)};

            // Now, how much do they need in order to be successful?
            STAmount const sunDeliver {optDeliverMin ?
                std::max (*optDeliverMin, std::min (sendMax, srcLiquid)) :
                ctx_.tx.getFieldAmount (sfAmount)};

            if (srcLiquid < sunDeliver)
            {
                // Vote no. However the transaction might succeed if applied
                // in a different order.
                JLOG(j_.trace()) << "Cash Check: Insufficient SUN: "
                    << srcLiquid.getFullText()
                    << " < " << sunDeliver.getFullText();
                return tecUNFUNDED_PAYMENT;
            }

            if (optDeliverMin && doFix1623)
                // Set the DeliveredAmount metadata.
                ctx_.deliver (sunDeliver);

            // The source account has enough SUN so make the ledger change.
            transferSUN (psb, srcId, account_, sunDeliver, viewJ);
        }
        else
        {
