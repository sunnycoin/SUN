...
#include <sunnycoin/app/tx/impl/InvariantCheck.h>
    ...
    beast::Journal const& j)
{
    ...
    if (fee > tx.getFieldAmount(sfFee).sun())
    {
    ...
    std::shared_ptr <SLE const> const& after)
{
     ...
     * previous SUN values and add to the total any new SUN values. The net
    ...
    if(before)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            drops_ -= (*before)[sfBalance].sun().drops();
            break;
        case ltPAYCHAN:
            drops_ -= ((*before)[sfAmount] - (*before)[sfBalance]).sun().drops();
            break;
        case ltESCROW:
            drops_ -= (*before)[sfAmount].sun().drops();
            break;
        default:
            break;
        }
    ...
    if(after)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            drops_ += (*after)[sfBalance].sun().drops();
            break;
        case ltPAYCHAN:
            if (! isDelete)
                drops_ += ((*after)[sfAmount] - (*after)[sfBalance]).sun().drops();
            break;
        case ltESCROW:
            if (! isDelete)
                drops_ += (*after)[sfAmount].sun().drops();
            break;
        default:
            break;
        }
    ...
    beast::Journal const& j)
{
    ...
    // transaction created SUN out of thin air. That's not possible.
    if (drops_ > 0)
    {
        JLOG(j.fatal()) <<
            "Invariant failed: SUN net change was positive: " << drops_;
        return false;
    }
    ...
    {
        JLOG(j.fatal()) <<
            "Invariant failed: SUN net change of " << drops_ <<
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& balance)
    {
        ...
        auto const drops = balance.sun().drops();
...
bool
SUNBalanceChecks::finalize(STTx const&, TER const, SUNAmount const, beast::Journal const& j)
{
    if (bad_)
    {
        JLOG(j.fatal()) << "Invariant failed: incorrect account SUN balance";
        return false;
    }
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& pays, STAmount const& gets)
    {
        ...
        // Can't have an SUN to SUN offer:
...
bool
NoBadOffers::finalize(STTx const& tx, TER const, SUNAmount const, beast::Journal const& j)
{
    if (bad_)
    {
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& amount)
    {
        ...
        if (amount.sun().drops() <= 0)
            return true;

        if (amount.sun().drops() >= SYSTEM_CURRENCY_START)
            return true;
...
bool
NoZeroEscrow::finalize(STTx const& tx, TER const, SUNAmount const, beast::Journal const& j)
{
    if (bad_)
    {
...
bool
AccountRootsNotDeleted::finalize(STTx const&, TER const, SUNAmount const, beast::Journal const& j)
{
    if (! accountDeleted_)
        return true;
...
bool
LedgerEntryTypesMatch::finalize(STTx const&, TER const, SUNAmount const, beast::Journal const& j)
{
...
void
NoSUNTrustLines::visitEntry(
    uint256 const&,
    bool,
    std::shared_ptr <SLE const> const&,
    std::shared_ptr <SLE const> const& after)
{
    if (after && after->getType() == ltSUNNYCOIN_STATE)
    {
        // checking the issue directly here instead of
        // relying on .native() just in case native somehow
        // were systematically incorrect
        sunTrustLine_ =
            after->getFieldAmount (sfLowLimit).issue() == sunIssue() ||
            after->getFieldAmount (sfHighLimit).issue() == sunIssue();
    }
}

bool
NoSUNTrustLines::finalize(STTx const&, TER const, SUNAmount const, beast::Journal const& j)
{
    if (! sunTrustLine_)
        return true;

    JLOG(j.fatal()) << "Invariant failed: an SUN trust line was created";
    return false;
}
