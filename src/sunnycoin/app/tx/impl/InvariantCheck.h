...
#ifndef SUNNYCOIN_APP_TX_INVARIANTCHECK_H_INCLUDED
#define SUNNYCOIN_APP_TX_INVARIANTCHECK_H_INCLUDED
...
public:
    ...
    bool
    finalize(STTx const&, TER const, SUNAmount const, beast::Journal const&);
};

/**
 * @brief Invariant: A transaction must not create SUN and should only destroy
 * the SUN fee.
 *
 * We iterate through all account roots, payment channels and escrow entries
 * that were modified and calculate the net change in SUN caused by the
 * transactions.
 */
class SUNNotCreated
{
...
/**
 * @brief Invariant: An account SUN balance must be in SUN and take a value
 ...
 * their SUN balances are reasonable.
 */
class SUNBalanceChecks
{
    bool bad_ = false;
 ...
 * no offers which contain negative amounts or which exchange SUN for SUN.
 */
class NoBadOffers
{
    bool bad_ = false;
 ...
 * methods
 *
 * @see sunnycoin::InvariantChecker_PROTOTYPE
 */
inline
InvariantChecks
getInvariantChecks()
