...
#ifndef SUNNYCOIN_APP_BOOK_TAKER_H_INCLUDED
#define SUNNYCOIN_APP_BOOK_TAKER_H_INCLUDED

#include <sunnycoin/app/tx/impl/Offer.h>
...
protected:
    ...
    struct Flow
    {
        ...
        bool sanity_check () const
        {
            ...
            if (isSUN (order.in) && isSUN (order.out))
                return false;
...
private:
    ...
    Flow
    flow_sun_to_iou (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_out);

    Flow
    flow_iou_to_sun (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_in);
...
private:
    ...
    TER
    transferSUN (AccountID const& from, AccountID const& to, STAmount const& amount);
...
private:
    ...
    // The amount of SUN that flowed if we were autobridging
    STAmount sun_flow_;
