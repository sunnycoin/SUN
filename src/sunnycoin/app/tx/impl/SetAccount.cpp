...
SetAccount::preflight (PreflightContext const& ctx)
{
    ...
    // DisallowSUN
    //
    bool bSetDisallowSUN   = (uTxFlags & tfDisallowSUN) || (uSetFlag == asfDisallowSUN);
    bool bClearDisallowSUN = (uTxFlags & tfAllowSUN) || (uClearFlag == asfDisallowSUN);

    if (bSetDisallowSUN && bClearDisallowSUN)
    {
        JLOG(j.trace()) << "Malformed transaction: Contradictory flags set.";
        return temINVALID_FLAG;
    }
...
SetAccount::doApply ()
{
    ...
    bool const bSetDisallowSUN   {(uTxFlags & tfDisallowSUN) || (uSetFlag == asfDisallowSUN)};
    bool const bClearDisallowSUN {(uTxFlags & tfAllowSUN) || (uClearFlag == asfDisallowSUN)};
    ...
    // DisallowSUN
    //
    if (bSetDisallowSUN && !(uFlagsIn & lsfDisallowSUN))
    {
        JLOG(j_.trace()) << "Set lsfDisallowSUN.";
        uFlagsOut |= lsfDisallowSUN;
    }

    if (bClearDisallowSUN && (uFlagsIn & lsfDisallowSUN))
    {
        JLOG(j_.trace()) << "Clear lsfDisallowSUN.";
        uFlagsOut &= ~lsfDisallowSUN;
    }
    ...
    // DefaultSUNNYCoin
    //
    if (uSetFlag == asfDefaultSUNNYCoin)
    {
        JLOG(j_.trace()) << "Set lsfDefaultSUNNYCoin.";
        uFlagsOut |= lsfDefaultSUNNYCoin;
    }
    else if (uClearFlag == asfDefaultSUNNYCoin)
    {
        JLOG(j_.trace()) << "Clear lsfDefaultSUNNYCoin.";
        uFlagsOut &= ~lsfDefaultSUNNYCoin;
    }
