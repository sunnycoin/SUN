...
#include <sunnycoin/app/tx/impl/CreateOffer.h>
...
#include <sunnycoin/beast/utility/WrappedSink.h>
...
CreateOffer::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.debug()) <<
            "Malformed offer: redundant (SUN for SUN)";
        return temBAD_OFFER;
    }
    ...
    // We don't allow a non-native currency to use the currency code SUN.
...
CreateOffer::reachedOfferCrossingLimit (Taker const& taker) const
{
    ...
    // The crossing limit is part of the SUNNYCoin protocol and
    ...
    NetClock::time_point const when)
{
    ...
    assert (!isSUN (takerAmount.in) && !isSUN (takerAmount.out));

    if (isSUN (takerAmount.in) || isSUN (takerAmount.out))
        Throw<std::logic_error> ("Bridging with SUN and an endpoint.");
    ...
    OfferStream offers_leg1 (view, view_cancel,
        Book (taker.issue_in (), sunIssue ()),
        when, stepCounter_, j_);

    OfferStream offers_leg2 (view, view_cancel,
        Book (sunIssue (), taker.issue_out ()),
        when, stepCounter_, j_);
    ...
    Amounts const& takerAmount)
{
    ...
    //
    // We check this in preclaim, but when selling SUN
    ...
    Amounts const& takerAmount)
{
    try
    {
        ...
        //
        // We check this in preclaim, but when selling SUN charged fees can
        ...
        // of the takerAmount currencies are SUN then we cross through an
        // additional path with SUN as the intermediate between two books.
...
static std::string to_string (SBoxCmp c)
{
    ...
    case SBoxCmp::sunRound:
        return "SUN round to zero";
    case SBoxCmp::diff:
        return "different";
    }
    return {};
    ...
    beast::Journal j)
{
    ...
    if (diff.hasDiff())
    {
        ...
        // There is a special case of an offer with SUN on one side where
        // the SUN gets rounded to zero.  It mostly looks like dust-level
        // differences.  It is easier to detect if we look for it before
        // removing the dust differences.
        if (int const side = diff.sunRoundToZero())
        {
            char const* const whichSide = side > 0 ? "; Flow" : "; Taker";
            j.stream (kWarning) << "FlowCross: " << name << " different" <<
                whichSide << " SUN rounded to zero.  tx: " <<
                ctx.tx.getTransactionID();
            return SBoxCmp::sunRound;
        }
...
CreateOffer::preCompute()
{
    ...
    if (pays_sun && !gets_sun)
        cross_type_ = CrossType::IouToJrp;
    else if (gets_sun && !pays_sun)
...
CreateOffer::applyGuts (Sandbox& sb, Sandbox& sbCancel)
{
    ...
    {
        SUNAmount reserve = ctx_.view().fees().accountReserve(
