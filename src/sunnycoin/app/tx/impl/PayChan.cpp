...
#include <sunnycoin/app/tx/impl/PayChan.h>
#include <sunnycoin/basics/chrono.h>
#include <sunnycoin/basics/Log.h>
#include <sunnycoin/ledger/ApplyView.h>
...
/*
    PaymentChannel
        ...
        Payment channels permit off-ledger checkpoints of SUN payments flowing
        in a single direction. A channel sequesters the owner's SUN in its own
    ...
    PaymentChannelCreate
        ...
        Amount
            The amount of SUN to deposit in the channel immediately.
        SettleDelay
        ...
        DestinationTag (optional)
            ...
            Wallet to be mapped back onto the SUNNYCoin ledger. The destination tag
        ...
        SourceTag (optional)
            ...
            to be mapped back onto the SUNNYCoin ledger. Source tags are similar to
    ...
    PaymentChannelClaim
        ...
        Channel
            The 256-bit ID of the channel.
        Balance (optional)
            The total amount of SUN delivered after this claim is processed (optional, not
            needed if just closing).
        Amount (optional)
            The amount of SUN the signature is for (not needed if equal to Balance or just
            closing the line).
        Signature (optional)
...
PayChanCreate::preflight (PreflightContext const& ctx)
{
    ...
    if (!isSUN (ctx.tx[sfAmount]) || (ctx.tx[sfAmount] <= beast::zero))
...
PayChanCreate::preclaim(PreclaimContext const &ctx)
{
...

    {
        ...
        // Obeying the lsfDisallowSUN flag was a bug.  Piggyback on
        // featureDepositAuth to remove the bug.
        if (!ctx.view.rules().enabled(featureDepositAuth) &&
            ((*sled)[sfFlags] & lsfDisallowSUN))
            return tecNO_TARGET;
    }
...
//  bool const noTecs = ctx.rules.enabled(fix1512);
    ...
    if (bal && (!isSUN (*bal) || *bal <= beast::zero))
        return temBAD_AMOUNT;

    auto const amt = ctx.tx[~sfAmount];
    if (amt && (!isSUN (*amt) || *amt <= beast::zero))
        return temBAD_AMOUNT;
    ...
    if (auto const sig = ctx.tx[~sfSignature])
    {
        ...
        auto const reqBalance = bal->sun ();
        auto const authAmt = amt ? amt->sun() : reqBalance;
...
PayChanClaim::doApply()
{
    ...
    {
        auto const chanBalance = slep->getFieldAmount (sfBalance).sun ();
        auto const chanFunds = slep->getFieldAmount (sfAmount).sun ();
        auto const reqBalance = ctx_.tx[sfBalance].sun ();
        ...
        if (!depositAuth &&
            (txAccount == src && (sled->getFlags() & lsfDisallowSUN)))
            return tecNO_TARGET;
        ...
        SUNAmount const reqDelta = reqBalance - chanBalance;
