...
#ifndef SUNNYCOIN_TX_APPLYSTEPS_H_INCLUDED
#define SUNNYCOIN_TX_APPLYSTEPS_H_INCLUDED
...
struct TxConsequences
{
    ...
    SUNAmount const potentialSpend;

    /// Constructor
    TxConsequences(ConsequenceCategory const category_,
        SUNAmount const fee_, SUNAmount const spend_)
        : category(category_)
        , fee(fee_)
...
/** Determine the SUN balance consequences if a transaction
    consumes the maximum SUN allowed.
