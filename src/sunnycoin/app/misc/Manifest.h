...
#ifndef SUNNYCOIN_APP_MISC_MANIFEST_H_INCLUDED
#define SUNNYCOIN_APP_MISC_MANIFEST_H_INCLUDED
...
/*
    ...
    Suppose the secret keys installed on a SUNNYCoin validator are compromised.  Not
    only do you have to generate and install new key pairs on each validator,
    EVERY sunnycoind needs to have its config updated with the new public keys, and
    ...
    received from sunnycoind peers.
    ...
    signed by the master key.  When a sunnycoind peer receives the new manifest,
