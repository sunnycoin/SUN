...
SUNNYCoind's fee mechanism consists of several interrelated processes:
...
for the queue if it meets these additional criteria:
  ...
  * all other queued transactions for that account, in the case where
  they spend the maximum possible SUN, leave enough SUN balance to pay
  the fee,
  * the total fees for the other queued transactions are less than both
  the network's minimum reserve and the account's SUN balance, and
...
This demonstrates that a simpler transaction paying less SUN can be more
likely to get into the open ledger, or be sorted earlier in the queue
than a more complex transaction paying more SUN.
...
more (or less) work for sunnycoind to process. Those transactions may have
...
behavior of the sunnycoin network. Note that this is not necessarily the
...
automatically as the sunnycoin network's performance improves, allowing
...
not make sense if sunnycoind is not synced to the network.
...
### [`server_info`](https://sunnycoin.com/build/sunnycoind-apis/#server-info) command
...
### [`server_state`](https://sunnycoin.com/build/sunnycoind-apis/#server-state) command
