    ...
    TxQ::Setup const& setup)
{
    ...
    // happen.
    return mulDiv(tx[sfFee].sun().drops(),
        baseRefLevel,
...
/*
    ...
    0. Is `featureFeeEscalation` enabled?
        ...
        No: Fallback to `sunnycoin::apply`. Stop.
        ...
        ApplyFlags flags, beast::Journal j)
{
    ...
    if (!allowEscalation)
    {
        return sunnycoin::apply(app, view, *tx, flags, j);
    }
    ...
    struct MultiTxn
    {
        ...
        SUNAmount fee = beast::zero;
        SUNAmount potentialSpend = beast::zero;
        ...
        if (sle)
        {
            ...
            if (multiTxn)
            {
                /* Check if the total fees in flight are greater
                    ...
                    Assume: Minimum account reserve is 20 SUN.
                    Example 1: If I have 1,000,000 SUN, I can queue
                        a transaction with a 1,000,000 SUN fee. In
                        the meantime, some other transaction may
                        lower my balance (eg. taking an offer). When
                        the transaction executes, I will either
                        spend the 1,000,000 SUN, or the transaction
                        will get stuck in the queue with a
                        `terINSUF_FEE_B`.
                    Example 2: If I have 1,000,000 SUN, and I queue
                        10 transactions with 0.1 SUN fee, I have 1 SUN
                        in flight. I can now queue another tx with a
                        999,999 SUN fee. When the first 10 execute,
                        ...
                        999,999 SUN, or get stuck in the queue.
                    Example 3: If I have 1,000,000 SUN, and I queue
                        7 transactions with 3 SUN fee, I have 21 SUN
                ...
                */
                auto const balance = (*sle)[sfBalance].sun();
                auto totalFee = multiTxn->fee;
                if (multiTxn->includeCurrentFee)
                    totalFee += (*tx)[sfFee].sun();
                if (totalFee >= balance ||
