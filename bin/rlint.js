...
var Remote      = require('sunnycoin-lib').Remote;
var Transaction = require('sunnycoin-lib').Transaction;
var UInt160     = require('sunnycoin-lib').UInt160;
var Amount      = require('sunnycoin-lib').Amount;
...
//  console.log(JSON.stringify(dir_entry, undefined, 2));
  ...
  ledger.accountState.forEach(function (entry) {
      ...
      else if (entry.LedgerEntryType === 'SUNNYCoinState')
      {
        ...
        if (entry.HighLimit.issuer === entry.LowLimit.issuer)
          sunnycoin_selfs[entry.Account] = entry;
      }
  ...
  if (Object.keys(sunnycoin_selfs).length)
    console.log("SUNNYCoinState selfs = %s", Object.keys(sunnycoin_selfs).length);
