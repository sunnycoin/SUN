...
sunnycoind_exe=/opt/sunnycoin/bin/sunnycoind
conf_file=/etc/opt/sunnycoin/sunnycoind.cfg

while getopts ":e:c:" opt; do
    case $opt in
        e)
            sunnycoind_exe=${OPTARG}
            ;;
        c)
            conf_file=${OPTARG}
            ;;
...
tmp_loc=$(mktemp -d --tmpdir sunnycoin_info.XXXX)
cd /tmp
chmod 751 sunnycoin_info.*
cd ~
echo ${tmp_loc}

cleaned_conf=${tmp_loc}/cleaned_sunnycoind_cfg.txt
...
echo "Use the following command on your local machine to download from your sunnycoind instance: scp <remote_sunnycoind_username>@<remote_host>:${tmp_loc}/info-package.tar.gz <path/to/local_machine/directory>"| tee /dev/fd/3
