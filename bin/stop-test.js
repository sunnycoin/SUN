...
if (process.argv[2] == null) {
  [
   'Usage: ',
   '',
   '  `node bin/stop-test.js i,j [sunnycoind_path] [sunnycoind_conf]`',
   '',
   '  Launch sunnycoind and stop it after n seconds for all n in [i, j}',
   '  For all even values of n launch sunnycoind with `--fg`',
   '  For values of n where n % 3 == 0 launch sunnycoind with `--fg`\n',
   'Examples: ',
   '',
   '  $ node bin/stop-test.js 5,10',
   ('  $ node bin/stop-test.js 1,4 ' +
      'build/clang.debug/sunnycoind $HOME/.confs/sunnycoind.cfg')
   ]
  ...
  var sunnycoindPath = process.argv[3] || 'build/sunnycoind'
  var sunnycoindConf = process.argv[4] || 'sunnycoind.cfg'
}
...
// default args
var conf_args = ['--conf='+sunnycoindConf];
...
function start(args) {
    return child.spawn(sunnycoindPath, args, options);
}
function stop(sunnycoind) { child.execFile(sunnycoindPath, stop_args, options)}
...
(function oneTest(n /* seconds */, results) {
  ...
  console.log("\nLaunching `%s` with `%s` for %d seconds",
                sunnycoindPath, JSON.stringify(args), n);

  sunnycoind = start(args);
  console.log("SUNNYCoind pid: %d", sunnycoind.pid);
  ...
  secs_l8r(n, function(){
    console.log("Stopping sunnycoind after %d seconds", n);
    ...
    if (process.env.ABORT_TESTS_ON_STALL != null) {
      ...
      secs_l8r(30, function() {
        if (result.stop_took == null) {
          console.log("sunnycoind has stalled");
          process.exit();
        };
      });
    }
