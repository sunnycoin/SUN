...
# Ensure APP defaults to sunnycoind if it's not set.
: ${APP:=sunnycoind}
echo "using APP: $APP"
...
# in sunnycoind that we want to run
declare -a manual_tests=(
  "beast.chrono.abstract_clock"
  "beast.unit_test.print"
  "sunnycoin.NodeStore.Timing"
  "sunnycoin.app.Flow_manual"
  "sunnycoin.app.NoSUNNYCoinCheckLimits"
  "sunnycoin.app.PayStrandAllPairs"
  "sunnycoin.consensus.ByzantineFailureSim"
  "sunnycoin.consensus.DistributedValidators"
  "sunnycoin.consensus.ScaleFreeSim"
  "sunnycoin.sunnycoin_data.digest"
  "sunnycoin.tx.CrossingLimits"
  "sunnycoin.tx.FindOversizeCross"
  "sunnycoin.tx.Offer_manual"
  "sunnycoin.tx.OversizeMeta"
  "sunnycoin.tx.PlumpBook"
)

: ${APP_ARGS:=}
if [[ ${APP} == "sunnycoind" ]]; then
  if [[ ${MANUAL_TESTS:-} == true ]]; then
    APP_ARGS+=" --unittest=$(join_by , "${manual_tests[@]}")"
  else
    APP_ARGS+=" --unittest --quiet --unittest-log"
  fi
  # Only report on src/sunnycoin files
  export LCOV_FILES="*/src/sunnycoin/*"
