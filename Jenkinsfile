...
//
github_api = 'https://api.github.com/repos/sunnycoin/sunnycoind'
...
"""
    ...
    stage ('Parallel Build') {
        ...
        for (int index = 0; index < variants.size(); index++) {
            ...
            def node_type =
                (compiler == 'msvc') ? 'sunnycoind-win' : 'sunnycoind-dev'
            ...
            builds[bldlabel] = {
                node(node_type) {
                    ...
                    withCredentials(
                        [string(
                            credentialsId: 'SUNNYCOIND_CODECOV_TOKEN',
        ...
        builds['rpm'] = {
            node('docker') {
                def bldlabel = 'rpm'
                def remote =
                    (git_fork == 'sunnycoin') ? 'origin' : git_fork

                withCredentials(
                    [string(
                        credentialsId: 'SUNNYCOIND_RPM_ROLE_ID',
                        variable: 'ROLE_ID')])
                {
                    withEnv([
                        'docker_image=artifactory.ops.sunnycoin.com:6555/sunnycoind-rpm-builder:latest',
...
# Vault Steps
SECRET_ID=$(cat /.vault/sunnycoind-build-role/secret-id)
export VAULT_TOKEN=$(/usr/local/sunnycoin/ops-toolbox/vault/vault_approle_auth -r ${ROLE_ID} -s ${SECRET_ID} -t)
/usr/local/sunnycoin/ops-toolbox/vault/vault_get_sts_token.py -r sunnycoind-build-role
...
docker run --rm \
-v $PWD/rpm-out:/opt/sunnycoind-rpm/out \
...
rpm -i /opt/sunnycoind-rpm/*.rpm
rc=$?; if [[ $rc != 0 ]]; then
  error "error installing rpms"
fi

/opt/sunnycoin/bin/sunnycoind --unittest
rc=$?; if [[ $rc != 0 ]]; then
  error "sunnycoind --unittest failed"
fi

/opt/sunnycoin/bin/validator-keys --unittest
rc=$?; if [[ $rc != 0 ]]; then
...
docker run --rm \
-v $PWD/rpm-out/rpm-main:/opt/sunnycoind-rpm \
-v $PWD:/opt/sunnycoind --entrypoint /opt/sunnycoind/test_rpm.sh \
centos:latest
'''
}
