...
[SUNNYCoin Consensus Algorithm](https://sunnycoin.com/files/sunnycoin/consensus/whitepaper.pdf)
as implemented in [sunnycoind](https://github.com/sunnycoin/sunnycoind), but
focuses on its utility as a generic consensus algorithm independent of the
detailed mechanics of the SUNNYCoin Consensus Ledger. Most notably, the algorithm
...
state.  For the SUNNYCoin network, the shared state is the current ledger--account
...
prior ledger.  For the SUNNYCoin network, transactions include payments,
...
The SUNNYCoin network is a decentralized and **trust-full** network.  Anyone is free
...
Leveraging this network of trust, the SUNNYCoin algorithm has two main components.
...
implementation in `sunnycoind`.  The document **does not** discuss correctness,
fault-tolerance or liveness properties of the algorithms or the full details of
how they integrate within `sunnycoind` to support the SUNNYCoin Consensus Ledger.
