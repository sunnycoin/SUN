## Heap profiling of sunnycoind with jemalloc
...
jemalloc is acquired separately from sunnycoind, and is not affiliated
with SUNNYCoin Labs. If you compile and install jemalloc from the
...
profiling has been tested with sunnycoind on a Linux platform. It should
work on platforms on which both sunnycoind and jemalloc are available.

To link sunnycoind with jemalloc, the argument
...
Build sunnycoind with jemalloc library under /usr/local/lib and
header under /usr/local/include:

    $ scons profile-jemalloc=/usr/local

Build sunnycoind using clang with the jemalloc library under /opt/local/lib
